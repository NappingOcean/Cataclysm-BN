...
auto isolated_city_tiles_in_radius(const tripoint_om_omt& center, const int radius)
    ...
    for (const auto x : iota(center.x() - radius, center.x() + radius + 1)) {
        for (const auto y : iota(center.y() - radius, center.y() + radius + 1)) {
            ...
            if (distance <= radius) { result.push_back({.pos = candidate, .distance = distance}); }
        }
    }
...
auto isolated_city_special_candidate_allowed(
    const isolated_city_tile& tile, const isolated_city_special_placement_options& opts) -> bool {
    if (opts.placement == "center") { return tile.pos == opts.center; }
...
auto isolated_city_lake_column_or_default_terrain(const oter_id& oter, const oter_id& default_oter)
    -> bool {
...
auto isolated_city_coastline_adjustment(
    const tripoint_om_omt& center, const tripoint_om_omt& pos, const int variance) -> int {
    ...
    const auto wobble =
        std::sin(angle * 2.0 + phase_a) * 0.55 + std::sin(angle * 5.0 + phase_b) * 0.35
        + std::sin(local) * 0.10;
    return std::clamp(static_cast<int>(std::lround(wobble * variance)), -variance, variance);
}
...
auto isolated_city_nearest_development_distance(
    ...
    for (const auto& dev : developed) {
        ...
        if (!nearest.has_value() || distance < *nearest) { nearest = distance; }
    }
    return nearest;
}
...
////////////////
oter_id ot_null, ot_crater, ot_field, ot_forest, ot_forest_thick, ot_forest_water, ot_river_center;
...
template <>
auto enum_to_string<overmap_special_subtype>(overmap_special_subtype data) -> std::string {
    switch (data) {
        // *INDENT-OFF*
...
template <> auto enum_to_string<om_direction::type>(om_direction::type d) -> std::string {
    switch (d) {
        // *INDENT-OFF*
...
template <> auto enum_to_string<cube_direction>(cube_direction data) -> std::string {
    switch (data) {
        // *INDENT-OFF*
...
constexpr auto rotate(size_t line, om_direction::type dir) -> size_t {
    if (dir == om_direction::type::invalid) { return line; }
    // Bitwise rotation to the left.
    return (
        ((line << static_cast<size_t>(dir))
         | (line >> (om_direction::size - static_cast<size_t>(dir))))
        & om_direction::bits);
}

constexpr auto set_segment(size_t line, om_direction::type dir) -> size_t {
    if (dir == om_direction::type::invalid) { return line; }
    return line | 1 << static_cast<size_t>(dir);
}

constexpr auto has_segment(size_t line, om_direction::type dir) -> bool {
    if (dir == om_direction::type::invalid) { return false; }
    return static_cast<bool>(line & 1 << static_cast<size_t>(dir));
}

constexpr auto is_straight(size_t line) -> bool {
    return line == 1 || line == 2 || line == 4 || line == 5 || line == 8 || line == 10;
}

static auto from_dir(om_direction::type dir) -> size_t {
    switch (dir) {
        case om_direction::type::north:
        case om_direction::type::south:
            return 5; // ns;
...
city::city(const point_om_omt& P, int const S): pos(P), size(S), name(Name::get(nameIsTownName)) {}

auto city::get_distance_from(const tripoint_om_omt& p) const -> int {
...
auto operator==(const int_id<oter_t>& lhs, const char* rhs) -> bool {
    return lhs.id().str() == rhs;
}

auto operator!=(const int_id<oter_t>& lhs, const char* rhs) -> bool { return !(lhs == rhs); }
...
auto overmap_land_use_code::get_symbol() const -> std::string { return utf32_to_utf8(symbol); }
...
void overmap_land_use_codes::check_consistency() { land_use_codes.check(); }

void overmap_land_use_codes::reset() { land_use_codes.reset(); }

auto overmap_land_use_codes::get_all() -> const std::vector<overmap_land_use_code>& {
...
void overmap_special_connection::deserialize(const JsonObject& jo) {
    ...
    if (jo.has_string("terrain")) {
        ...
        if (json_report_strict) {
            try {
                jo.throw_error(
                    "Defining connection by terrain is deprecated, use explicit 'connection' instead.",
                    "terrain");
            } catch (const JsonError& e) { debugmsg("%s", e.what()); }
        }
...
void overmap_specials::check_consistency() { specials.check(); }

void overmap_specials::reset() { specials.reset(); }

auto overmap_specials::get_all() -> const std::vector<overmap_special>& {
    return specials.get_all();
}

auto overmap_specials::get_default_batch(const point_abs_om& origin) -> overmap_special_batch {
    std::vector<const overmap_special*> res;

    res.reserve(specials.size());
    for (const overmap_special& elem : specials.get_all()) {
        if (elem.can_spawn()) { res.push_back(&elem); }
    }

    return overmap_special_batch(origin, res);
}

auto is_river(const oter_id& ter) -> bool { return ter->is_river(); }

auto is_river_or_lake(const oter_id& ter) -> bool {
    return ter->is_river() || ter->is_lake() || ter->is_lake_shore();
}

auto is_ot_match(const std::string& name, const oter_id& oter, const ot_match_type match_type)
    -> bool {
    ...
    static const auto is_ot_type = [](const std::string& otype, const oter_id& oter) {
        // Is a match if the base type is the same which will allow for handling rotations/linear
        // features but won't incorrectly match other locations that happen to contain the
        // substring.
    ...
    static const auto is_ot_prefix = [](const std::string& otype, const oter_id& oter) {
        ...
        if (compare_size > oter_size) { return false; }

        const auto& oter_str = oter.id();
        if (oter_str.str().compare(0, compare_size, otype) != 0) { return false; }

        // check if it's a full match
        if (compare_size == oter_size) { return true; }
 ...
 * suffix is for roads/subways/etc which have "_straight", "_curved", "_tee", "_four_way" function
 * mappings
 */
static void load_overmap_terrain_mapgens(
    const JsonObject& jo, const std::string& id_base, const std::string& suffix = "") {
...
auto oter_type_t::get_symbol() const -> std::string { return utf32_to_utf8(symbol); }
...
void oter_type_t::load(const JsonObject& jo, const std::string& src) {
    ...
    if (jo.has_string("map_label")) { map_label = jo.get_string("map_label"); }
    ...
    if (has_flag(oter_flags::line_drawing)) {
        ...
        if (!jo.has_string("sym") && jo.has_number("sym")) {
            debugmsg("sym is defined as number instead of string for overmap_terrain %s (%s)",
                     id.c_str(), name);
        }
...
void oter_type_t::register_terrain(const oter_t& peer, size_t n, size_t max_n) {
    ...
    if (peer.id.is_valid()) {
        ...
        debugmsg("Can't register the new overmap terrain \"%s\". It already exists.",
                 peer.id.c_str());
    } else {
...
auto oter_type_t::get_first() const -> oter_id {
    assert(!directional_peers.empty());
    return directional_peers.front();
}

auto oter_type_t::get_rotated(om_direction::type dir) const -> oter_id {
...
auto oter_type_t::get_linear(size_t n) const -> oter_id {
    if (!has_flag(oter_flags::line_drawing)) {
...
oter_t::oter_t(const oter_type_t& type, om_direction::type dir)
    : type(&type),
      ...
      symbol_alt(om_direction::rotate_symbol(
          type.land_use_code ? type.land_use_code->symbol : type.symbol, dir)),
...
auto oter_t::get_rotation() const -> int {
    if (is_linear()) {
        const om_lines::type& t = om_lines::all[line];
    ...
    if (is_rotatable()) { return static_cast<int>(get_dir()); }
    return 0;
}

auto oter_t::type_is(const oter_type_id& type_id) const -> bool { return type->id.id() == type_id; }

auto oter_t::type_is(const oter_type_t& type) const -> bool { return this->type == &type; }

auto oter_t::has_connection(om_direction::type dir) const -> bool {
    // TODO: It's a DAMN UGLY hack. Remove it as soon as possible.
    static const oter_str_id road_manhole("road_nesw_manhole");
    if (id == road_manhole) { return true; }
    return om_lines::has_segment(line, dir);
}

auto oter_t::is_hardcoded() const -> bool {
    // TODO: This set only exists because so does the monstrous 'if-else' statement in @ref
    // map::draw_map(). Get rid of both.
...
void overmap_terrains::check_consistency() {
    for (const auto& elem : terrain_types.get_all()) {
        ...
        if (elem.static_spawns.group && !elem.static_spawns.group.is_valid()) {
            debugmsg("Invalid monster group \"%s\" in spawns of \"%s\".",
                     elem.static_spawns.group.c_str(), elem.id.c_str());
        }
    }
    ...
    for (const auto& elem : terrains.get_all()) {
        ...
        if (has_mapgen_for(mid)) {
            if (test_mode && exists_hardcoded) {
                debugmsg(
                    "Mapgen terrain \"%s\" exists in both JSON and a hardcoded function.  Consider removing the latter.",
                    mid.c_str());
            }
...
void overmap_terrains::finalize() {
    ...
    if (!region_settings_map.contains("default")) {
        debugmsg(
            "ERROR: can't find default overmap settings (region_map_settings 'default'), "
            "cataclysm pending.  And not the fun kind.");
    }

    for (auto& elem : region_settings_map) { elem.second.finalize(); }
...
auto overmap_terrains::get_all() -> const std::vector<oter_t>& { return terrains.get_all(); }

static auto is_amongst_locations(
    const oter_id& oter, const cata::flat_set<overmap_location_id>& locations) -> bool {
    return std::any_of(locations.begin(), locations.end(), [&oter](const overmap_location_id& loc) {
        return loc->test(oter);
    });
}

auto overmap_special_locations::can_be_placed_on(const oter_id& oter) const -> bool {
...
void overmap_special_locations::deserialize(JsonIn& jsin) {
    ...
    if (ja.size() != 2) { ja.throw_error("expected array of size 2"); }
...
auto operator+(const cube_direction l, const om_direction::type r) -> cube_direction {
    return l + static_cast<int>(r);
}

auto operator+(const cube_direction d, int i) -> cube_direction {
    switch (d) {
        case cube_direction::north:
        case cube_direction::east:
        case cube_direction::south:
...
struct overmap_special_data {
    ...
    virtual auto get_terrain_at(const tripoint_rel_omt& p) const -> const oter_str_id& = 0;
    virtual auto all_terrains() const -> std::vector<oter_str_id> = 0;
    virtual auto preview_terrains() const -> std::vector<overmap_special_terrain> = 0;
    virtual auto required_locations() const -> std::vector<overmap_special_locations> = 0;
    virtual auto place(
        overmap& om, const tripoint_om_omt& origin, om_direction::type dir,
        const overmap_special& special) const -> special_placement_result = 0;
};
...
struct fixed_overmap_special_data: overmap_special_data {
    ...
    explicit fixed_overmap_special_data(const overmap_special_terrain& ter): terrains{ter} {}
    ...
    void finalize(
        ...
        for (auto& t : terrains) {
            if (t.locations.empty()) { t.locations = default_locations; }
        }
    }
        ...
        for (const overmap_special_terrain& elem : terrains) {
            ...
            if (elem.locations.empty()) {
                debugmsg("In %s, no location is defined for point %s or the "
                         "overall special.",
                         context, pos.to_string());
            }

            for (const auto& l : elem.locations) {
                if (!l.is_valid()) {
                    debugmsg("In %s, point %s, location \"%s\" is invalid.", context,
                             pos.to_string(), l.c_str());
                }
            }
        }
    }

    auto get_terrain_at(const tripoint_rel_omt& p) const -> const oter_str_id& override {
        const auto iter = std::
            find_if(terrains.begin(), terrains.end(), [&p](const overmap_special_terrain& elem) {
                return elem.p == p;
            });
        if (iter == terrains.end()) { return oter_str_id::NULL_ID(); }
        return iter->terrain;
    }

    auto all_terrains() const -> std::vector<oter_str_id> override {
        std::vector<oter_str_id> result;
        for (const overmap_special_terrain& ter : terrains) { result.push_back(ter.terrain); }
        return result;
    }

    auto preview_terrains() const -> std::vector<overmap_special_terrain> override {
        std::vector<overmap_special_terrain> result;
        std::copy_if(terrains.begin(), terrains.end(), std::back_inserter(result),
                     [](const overmap_special_terrain& terrain) { return terrain.p.z() == 0; });
        return result;
    }

    auto required_locations() const -> std::vector<overmap_special_locations> override {
    ...
    auto place(
        overmap& om, const tripoint_om_omt& origin, om_direction::type dir,
        const overmap_special& special) const -> special_placement_result override {
            ...
            if (blob) {
                for (int x = -2; x <= 2; x++) {
                    for (int y = -2; y <= 2; y++) {
                        ...
                        if (!overmap::inbounds(nearby_pos)) { continue; }
...
enum class join_type { mandatory, optional, available, reject, last };

template <> struct enum_traits<join_type> {
    static constexpr join_type last = join_type::last;
};

namespace io {

template <> auto enum_to_string<join_type>(join_type data) -> std::string {
    switch (data) {
        // *INDENT-OFF*
        case join_type::mandatory:
            return "mandatory";
        case join_type::optional:
            return "optional";
        case join_type::available:
            return "available";
        case join_type::reject:
            return "reject";
        // *INDENT-ON*
        case join_type::last:
            break;
...
struct z_constraints {
    enum class constraint_type { any, range, top, bottom, offset };

    constraint_type type = constraint_type::any;
    int min = INT_MIN;
    int max = INT_MAX;

    auto check(const tripoint_om_omt& pos, const int z_min, const int z_max) const -> bool {
        switch (type) {
    ...
    void deserialize(JsonIn& jsin) {
        if (jsin.test_int()) {
            ...
            JsonArray ja = jsin.get_array();
            if (ja.size() != 2) { ja.throw_error("Array should be in format [min, max]"); }
            min = ja.get_int(0);
            max = ja.get_int(1);
            type = constraint_type::range;
...
struct mutable_special_connection {
    ...
    void deserialize(const JsonObject& jo) { jo.read("connection", connection); }
...
struct mutable_overmap_terrain {
    ...
    void finalize(
        ...
        if (locations.empty()) { locations = default_locations; }
    ...
    void check(const std::string& context) const {
        ...
        if (locations.empty()) { debugmsg("In %s, no locations are defined", context); }
        ...
        for (const std::pair<const cube_direction, mutable_special_connection>& p : connections) {
            p.second.check(
                string_format("connection %s in %s", io::enum_to_string(p.first), context));
        }
    }
    ...
    void deserialize(JsonIn& jin) {
        ...
        for (int i = 0; i != static_cast<int>(cube_direction::last); ++i) {
            ...
            if (jo.has_member(dir_s)) { jo.read(dir_s, joins[dir], true); }
        }
...
struct mutable_overmap_placement_rule {
    ...
    void finalize(
        ...
        for (mutable_overmap_placement_rule_piece& piece : pieces) {
            ...
            if (!inserted) {
                debugmsg("phase of %s has chunk with duplicated position %s", context,
                         piece.pos.to_string());
            }
            auto it = special_overmaps.find(piece.overmap_id);
            if (it == special_overmaps.end()) {
                throw std::runtime_error(string_format(
                    "phase of %s specifies overmap %s which is not defined for that special",
        ...
        for (const mutable_overmap_placement_rule_piece& piece : pieces) {
            ...
            for (const join_map::value_type& p : ter.joins) {
                ...
                if (opposite_piece == pieces_by_pos.end()) {
                    ...
                    auto opposite_om_join = other_om.joins.find(other_side.dir - other_piece.rot);
                    if (opposite_om_join == other_om.joins.end()) {
                        debugmsg(
                            "in phase of %s, %s has adjacent pieces %s at %s and %s at "
                            ...
                            other_piece.overmap_id, other_piece.pos.to_string(), ter_join.join_id);
                        ...
                        debugmsg(
                            ...
                            context, description(), piece.overmap_id, piece.pos.to_string(),
                            other_piece.overmap_id, other_piece.pos.to_string(), ter_join.join_id,
                            opposite_join, opposite_om_join->second.join_id);
                    }
                }
            }
        }
    ...
    void check(
        ...
        if (pieces.empty()) {
            throw std::runtime_error(
                string_format("phase of %s has chunk with zero pieces", context));
        }
    ...
    auto realise(int scale) const -> mutable_overmap_placement_rule_remainder;
...
struct mutable_overmap_placement_rule_remainder {
    ...
    auto description() const -> std::string { return parent->description(); }

    auto get_weight() const -> int { return std::min(max, weight); }

    auto is_exhausted() const -> bool { return get_weight() == 0; }

    void decrement() { --max; }

    auto positions(om_direction::type rot) const -> std::vector<tripoint_rel_omt> {
    ...
    auto pieces(const tripoint_om_omt& origin, om_direction::type rot) const
        -> std::vector<mutable_overmap_piece_candidate> {
...
auto mutable_overmap_placement_rule::realise(int scale) const
    -> mutable_overmap_placement_rule_remainder {
...
public:
    ...
    auto any_unresolved() const -> bool { return !unresolved.empty(); }

    auto mandatory_amount_at(const tripoint_om_omt& pos) const -> int {
        int ret = 0;
        for (iterator it : unresolved.all_at(pos)) {
            if (it->type == join_type::mandatory) { ret++; }
        }
        return ret;
    }

    auto all_unresolved_at(const tripoint_om_omt& pos) const -> std::vector<const join*> {
        std::vector<const join*> result;
        for (iterator it : unresolved.all_at(pos)) { result.push_back(&*it); }
        return result;
    }

    auto is_finished() const -> bool {
        for (const auto& join : postponed) {
            if (join.type == join_type::mandatory) { return false; }
        }
        return true;
    }

    auto any_postponed_at(const tripoint_om_omt& p) const -> bool { return postponed.any_at(p); }
    ...
    auto allows(const om_pos_dir& this_side, const mutable_overmap_terrain_join& this_ter_join)
        const -> join_status {
        ...
        auto is_allowed_opposite = [&](const std::string& candidate) {
            ...
            if (this_join.opposite_id == candidate) { return true; }

            for (const mutable_overmap_join* alt_join : this_ter_join.alternative_joins) {
                if (alt_join->opposite_id == candidate) { return true; }
            }
        ...
        if (const join* existing = resolved.find(other_side)) {
            ...
            if (is_allowed_opposite(existing->join->id)) {
                ...
                } else if (other_type == join_type::reject
                           || this_ter_join.type == join_type::reject) {
                ...
                if (other_type == join_type::mandatory
                    || this_ter_join.type == join_type::mandatory) {
                    return join_status::disallowed;
                } else {
    ...
    void add_joins_for(
        const mutable_overmap_terrain& ter, const tripoint_om_omt& pos, om_direction::type rot,
        const std::vector<om_pos_dir>& suppressed_joins) {
        consistency_check();

        std::unordered_set<om_pos_dir> avoid(suppressed_joins.begin(), suppressed_joins.end());

        for (const std::pair<const cube_direction, mutable_overmap_terrain_join>& p : ter.joins) {
    ...
    auto pick_top_priority() const -> tripoint_om_omt {
        assert(any_unresolved());
        auto priority_it = std::find_if(
            unresolved_priority_index.begin(), unresolved_priority_index.end(),
            [](const cata::flat_set<iterator, compare_iterators>& its) { return !its.empty(); });
    ...
    void restore_postponed() {
        consistency_check();
        for (const join& j : postponed) { add_unresolved(j.where, j.type, j.join); }
        postponed.clear();
    }

    auto all_used() const -> const std::vector<std::pair<om_pos_dir, std::string>>& { return used; }
...
private:
    struct indexed_joins {
        ...
        auto begin() -> iterator { return joins.begin(); }

        auto end() -> iterator { return joins.end(); }

        auto begin() const -> const_iterator { return joins.begin(); }

        auto end() const -> const_iterator { return joins.end(); }

        auto empty() const -> bool { return joins.empty(); }

        auto count(const om_pos_dir& p) const -> bool { return position_index.contains(p); }

        auto find(const om_pos_dir& p) const -> const join* {
            auto it = position_index.find(p);
            if (it == position_index.end()) { return nullptr; }
            return &*it->second;
        }

        auto any_at(const tripoint_om_omt& pos) const -> bool {
            for (cube_direction dir : all_enum_values<cube_direction>()) {
                if (count(om_pos_dir{pos, dir})) { return true; }
            }
            return false;
        }

        auto all_at(const tripoint_om_omt& pos) const -> std::vector<iterator> {
            std::vector<iterator> result;
            for (cube_direction dir : all_enum_values<cube_direction>()) {
                om_pos_dir key{pos, dir};
                auto pos_it = position_index.find(key);
                if (pos_it != position_index.end()) { result.push_back(pos_it->second); }
            }
            return result;
        }

        auto add(const om_pos_dir& p, const join_type& t, const mutable_overmap_join* j)
            -> iterator {
            return add({p, t, j});
        }

        auto add(const join& j) -> iterator {
            joins.push_front(j);
            auto it = joins.begin();
    ...
    void add_unresolved(const om_pos_dir& p, const join_type& t, const mutable_overmap_join* j) {
        ...
        [[maybe_unused]] const bool inserted =
            unresolved_priority_index[priority].insert(it).second;
        assert(inserted);
    }

    auto erase_unresolved(const om_pos_dir& p) -> bool {
        auto pos_it = unresolved.position_index.find(p);
        if (pos_it == unresolved.position_index.end()) { return false; }
    ...
    struct compare_iterators {
        auto operator()(iterator l, iterator r) -> bool { return l->where < r->where; }
    };
...
struct mutable_overmap_phase_remainder {
    ...
    auto all_rules_exhausted() const -> bool {
        return std::all_of(
            rules.begin(), rules.end(), [](const mutable_overmap_placement_rule_remainder& rule) {
    ...
    struct can_place_result {
        ...
        auto as_pair() const -> std::pair<int, int> {
            return {num_context_mandatory_joins_matched, num_my_non_available_matched};
        }

        friend auto operator==(const can_place_result& l, const can_place_result& r) -> bool {
    ...
    auto can_place(
        ...
        const joins_tracker& unresolved) const -> std::optional<can_place_result> {
        ...
        for (const mutable_overmap_piece_candidate& piece : pieces) {
            if (!overmap::inbounds(piece.pos)) { return std::nullopt; }
            if (!is_amongst_locations(om.ter(piece.pos), piece.overmap->locations)) {
                return std::nullopt;
            }
            if (unresolved.any_postponed_at(piece.pos)) { return std::nullopt; }
        ...
        if (!have_required_join) { return std::nullopt; }
        ...
        for (const std::pair<om_pos_dir, const mutable_overmap_terrain_join*>& p :
            ...
            switch (unresolved.allows(pos_d, ter_join)) {
                ...
                case joins_tracker::join_status::matched_available:
                    if (ter_join.type == join_type::mandatory) { ++num_my_non_available_matched; }
                    continue;
                case joins_tracker::join_status::mismatched_available:
                    suppressed_joins.push_back(pos_d);
                // fallthrough
                case joins_tracker::join_status::free:
                    if (join.id == rule.parent->required_join) { return std::nullopt; }
                    break;
            }
            if (ter_join.type != join_type::mandatory) { continue; }
            // Verify that the remaining joins lead to
            // suitable locations
            tripoint_om_omt neighbour = pos_d.p + displace(pos_d.dir);
            if (!overmap::inbounds(neighbour)) { return std::nullopt; }
        ...
        return can_place_result{
            context_mandatory_joins_shortfall, num_my_non_available_matched, suppressed_joins};
    }

    auto satisfy(
        const overmap& om, const tripoint_om_omt& pos, const joins_tracker& unresolved,
        const bool rotatable, const int z_min, const int z_max) -> satisfy_result {
        ...
        for (mutable_overmap_placement_rule_remainder& rule : rules) {
            ...
            if (!rule.parent->z.check(pos, z_min, z_max)) { continue; }

            for (om_direction::type dir : om_direction::all) {
                for (const tripoint_rel_omt& piece_pos : rule.positions(dir)) {
                    tripoint_om_omt origin = pos - piece_pos;

                    if (std::optional<can_place_result> result =
                            can_place(om, rule, origin, dir, unresolved)) {
                        if (best_result < *result) {
                            pos_dir_options.clear();
                            best_result = *result;
                        }
                        if (*result == best_result) {
                            pos_dir_options.push_back(satisfy_result{
                                origin, dir, &rule, result.value().supressed_joins, {}});
                        }
                    }
                }

                if (rule.parent->rotate ? !(*rule.parent->rotate) : !rotatable) { break; }
            }
        ...
        std::string joins_s = enumerate_as_string(
            unresolved.all_unresolved_at(pos), [](const joins_tracker::join* j) {
        ...
        if (satisfy_result* picked = options.pick()) {
            ...
            picked->description = string_format(
                ...
                om.ter(pos + point_south).id().str(), om.ter(pos + point_west).id().str(), joins_s);
            picked->rule->decrement();
            return *picked;
        } else {
            std::string rules_s = enumerate_as_string(
                rules, [](const mutable_overmap_placement_rule_remainder& rule) {
            ...
            std::string message = string_format(
                ...
                pos.to_string(), om.ter(pos).id().str(), om.ter(pos + point_north).id().str(),
                om.ter(pos + point_east).id().str(), om.ter(pos + point_south).id().str(),
                om.ter(pos + point_west).id().str(), joins_s, rules_s);
...
struct mutable_overmap_phase {
    ...
    auto realise(overmap& om, std::unordered_map<std::string, int> scales) const
        -> mutable_overmap_phase_remainder {
        std::vector<mutable_overmap_placement_rule_remainder> realised_rules;
        for (const mutable_overmap_placement_rule& rule : rules) {
            if (rule.om_pos && *rule.om_pos != om.pos()) { continue; }
    ...
    void deserialize(JsonIn& jin) { jin.read(rules, true); }
};

template <typename Tripoint> pos_dir<Tripoint> pos_dir<Tripoint>::opposite() const {
    switch (dir) {
        case cube_direction::north:
...
template <typename Tripoint> void pos_dir<Tripoint>::serialize(JsonOut& jsout) const {
    jsout.start_array();
    jsout.write(p);
    jsout.write(dir);
    jsout.end_array();
...
template <typename Tripoint> void pos_dir<Tripoint>::deserialize(JsonIn& jsin) {
    JsonArray ja = jsin.get_array();
    if (ja.size() != 2) { ja.throw_error("Expected array of size 2"); }
    ja.read(0, p);
    ja.read(1, dir);
...
template <typename Tripoint> bool pos_dir<Tripoint>::operator==(const pos_dir<Tripoint>& r) const {
...
struct mutable_overmap_special_data: overmap_special_data {
    ...
    void finalize(
        ...
        for (size_t i = 0; i != joins_vec.size(); ++i) {
            ...
            if (join.into_locations.empty()) { join.into_locations = default_locations; }
            join.priority = i;
    ...
    void check(const std::string& context) const override {
        if (joins_vec.size() != joins.size()) { debugmsg("duplicate join id in %s", context); }
        for (const mutable_overmap_join& join : joins_vec) {
            if (join.opposite) {
                if (join.opposite->opposite_id != join.id) {
                    debugmsg(
                        "in %1$s: join id %2$s specifies its opposite to be %3$s, but "
                        "the opposite of %3$s is %4$s, when it should match the "
                        "original id %2$s",
                        context, join.id, join.opposite_id, join.opposite->opposite_id);
                }
            } else {
                debugmsg("in %s: join id '%s' specified as opposite of '%s' not valid", context,
                         join.opposite_id, join.id);
            }
        }
    ...
    auto root_as_overmap_special_terrain() const -> overmap_special_terrain {
        auto it = overmaps.find(root);
        if (it == overmaps.end()) {
    ...
    auto all_terrains() const -> std::vector<oter_str_id> override {
        std::vector<oter_str_id> result;
        for (const auto& ter : overmaps) { result.push_back(ter.second.terrain); }
        return result;
    }
    ...
    auto place(
        ...
        for (const auto& dist : shared_dist) { scales[dist.first] = dist.second.sample(); }
        ...
        while (unresolved.any_unresolved()) {
            ...
            if (!unresolved.any_unresolved() || phase_remaining.all_rules_exhausted()) {
                ++current_phase;
                if (current_phase == phases.end()) { break; }
        ...
        if (!unresolved.is_finished()) {
            ...
            std::string joins = enumerate_as_string(
                unresolved.all_unresolved_at(p), [](const joins_tracker::join* dir_join) {
                    return string_format(
                        "%s: %s", io::enum_to_string(dir_join->where.dir), dir_join->join->id);
                });

            debugmsg(
                "Spawn of mutable special %s had unresolved joins.  Existing terrain "
                "at %s was %s; joins were %s\nComplete record of placement follows:\n%s",
                special.id.str(), p.to_string(), current_terrain.id().str(), joins,
                join(descriptions, "\n"));

            om.add_note(p, string_format("U:R;DEBUG: unresolved joins %s at %s placing %s", joins,
                                         p.to_string(), special.id.str()));
        }
...
auto overmap_special::can_spawn() const -> bool {
    if (get_constraints().occurrences.empty()) { return false; }

    const int city_size = get_option<int>("CITY_SIZE");
    return city_size != 0 || get_constraints().city_size.min <= city_size;
}

auto overmap_special::requires_city() const -> bool {
    return constraints_.city_size.min > 0
        || constraints_.city_distance.max < std::max(OMAPX, OMAPY);
}

auto overmap_special::can_belong_to_city(const tripoint_om_omt& p, const city& cit) const -> bool {
    return constraints_.city_distance.contains(cit.get_distance_from(p) - (cit.size));
}

auto overmap_special::longest_side() const -> int {
    ...
    auto min_max_x = std::minmax_element(
        req_locations.begin(), req_locations.end(),
...
auto overmap_special::get_terrain_at(const tripoint_rel_omt& p) const -> const oter_str_id& {
    return data_->get_terrain_at(p);
}

auto overmap_special::all_terrains() const -> std::vector<oter_str_id> {
...
auto overmap_special::preview_terrains() const -> std::vector<overmap_special_terrain> {
    ...
    for (const auto& nested : get_nested_specials()) {
        for (const auto& ter : nested.second->preview_terrains()) {
            ...
            if (rel_ter.p.z() == 0) { result.push_back(rel_ter); }
        }
    }
    return result;
}

auto overmap_special::required_locations() const -> std::vector<overmap_special_locations> {
...
auto overmap_special::place(overmap& om, const tripoint_om_omt& origin, om_direction::type dir)
    const -> special_placement_result {
    return data_->place(om, origin, dir, *this);
}

auto overmap_special::get_args(const mapgendata& md) const -> mapgen_arguments {
...
void overmap_special::load(const JsonObject& jo, const std::string& src) {
    ...
    switch (subtype_) {
        ...
        case overmap_special_subtype::mutable_: {
            ...
            for (JsonObject joc : jo.get_array("check_for_locations_area")) {
                ...
                if (from.z() > to.z()) { std::swap(from.z(), to.z()); }
        ...
        default:
            jo.throw_error(
                string_format("subtype %s not implemented", io::enum_to_string(subtype_)));
    }
    ...
    if (jo.has_array("dimensions")) {
        dimensions_.clear();
        for (const std::string& dim : jo.get_array("dimensions")) { dimensions_.push_back(dim); }
    }

    // Another hack
    if (!is_special) { flags_.insert("ELECTRIC_GRID"); }
}

auto overmap_special::can_spawn_in_dimension(
    const dimension_id& dim_id, bool dim_inherits_base) const -> bool {
...
void overmap_special::finalize() {
    const_cast<overmap_special_data&>(*data_)
        .finalize("overmap special " + id.str(), default_locations_);

    for (auto& elem : connections) { elem.finalize(); }
}
...
void overmap_special::check() const {
    ...
    for (const auto& elem : connections) {
        ...
        if (!ter.is_null()) {
            debugmsg("In overmap special \"%s\", connection at %s overwrites terrain.", id,
                     elem.p.to_string());
        }
        if (!elem.connection.is_valid()) {
            debugmsg("In overmap special \"%s\", connection at %s has invalid id \"%s\".", id,
                     elem.p.to_string(), elem.connection);
        }
        ...
        if (elem.from) {
            ...
            switch (calculated_direction) {
                ...
                default:
                    debugmsg(
                        "In overmap special \"%s\", connection %s is not directly north, east, south or west of the defined \"from\" %s.",
    ...
    std::function<std::optional<
        overmap_special_id>(const overmap_special_id, std::unordered_set<overmap_special_id>)>
        check_recursion =
            [&check_recursion](
                const overmap_special_id special, std::unordered_set<overmap_special_id> parents)
    ...
    if (recurse) {
        debugmsg("In overmap special \"%s\", nested special \"%s\" places itself recursively.", id,
                 *recurse);
    }
}

// *** BEGIN overmap FUNCTIONS ***
overmap::overmap(const point_abs_om& p, const dimension_id& dim_id): loc(p), dimension_id_(dim_id) {
    ...
    if (rsit == region_settings_map.end()) {
        ...
        if (rsit == region_settings_map.end()) {
            debugmsg("overmap %s: can't find region '%s' or 'default'", loc.to_string(),
                     rsettings_id.c_str());
...
auto overmap::populate(const dimension_id& dim_id, overmap_special_batch& enabled_specials)
    -> void {
    try {
...
auto overmap::populate(const dimension_id& dim_id) -> void {
    ...
    const bool should_whitelist =
        !overmap_feature_flag.whitelist.empty() || overmap_feature_flag.clear_whitelist;
    ...
    if (should_blacklist) {
        for (auto it = enabled_specials.begin(); it != enabled_specials.end();) {
            if (cata::sets_intersect(
                    overmap_feature_flag.blacklist, it->special_details->get_flags())) {
    ...
    if (should_whitelist) {
        for (auto it = enabled_specials.begin(); it != enabled_specials.end();) {
            if (cata::sets_intersect(
                    overmap_feature_flag.whitelist, it->special_details->get_flags())) {
                ++it;
            } else {
                it = enabled_specials.erase(it);
            }
...
auto overmap::get_default_terrain(int z) const -> oter_id {
    if (z == 0) {
...
void overmap::ter_set(const tripoint_om_omt& p, const oter_id& id) {
    if (!inbounds(p)) {
        /// TODO: Add a debug message reporting this, but currently there are way too many place
...
auto overmap::ter(const tripoint_om_omt& p) const -> const oter_id& {
    if (!inbounds(p)) {
        /// TODO: Add a debug message reporting this, but currently there are way too many place
        /// that would trigger it.
        return ot_null;
...
auto overmap::join_used_at(const om_pos_dir& p) -> std::string* {
    auto it = joins_used.find(p);
    if (it == joins_used.end()) { return nullptr; }
    return &it->second;
}

auto overmap::mapgen_args(const tripoint_om_omt& p) -> std::optional<mapgen_arguments>* {
    auto it = mapgen_args_index.find(p);
    if (it == mapgen_args_index.end()) { return nullptr; }
    return &mapgen_arg_storage[it->second];
}

auto overmap::get_mapgen_args_slot(const tripoint_om_omt& p) -> overmap::mapgen_args_slot {
    auto it = mapgen_args_index.find(p);
    if (it == mapgen_args_index.end()) { return {}; }
...
void overmap::sync_mapgen_args_init_flags() {
    ...
    for (size_t i = 0; i < mapgen_arg_storage.size(); ++i) {
        if (mapgen_arg_storage[i].has_value()) { mapgen_args_init_flags_[i] = 1; }
    }
}

auto overmap::seen(const tripoint_om_omt& p) -> bool& {
    if (!inbounds(p)) {
        nullbool = false;
        return nullbool;
    }
...
auto overmap::seen(const tripoint_om_omt& p) const -> bool {
    if (!inbounds(p)) { return false; }
    return layer[p.z() + OVERMAP_DEPTH].visible[p.x()][p.y()];
}

auto overmap::explored(const tripoint_om_omt& p) -> bool& {
    if (!inbounds(p)) {
        nullbool = false;
        return nullbool;
    }
...
auto overmap::is_explored(const tripoint_om_omt& p) const -> bool {
    if (!inbounds(p)) { return false; }
    return layer[p.z() + OVERMAP_DEPTH].explored[p.x()][p.y()];
}

auto overmap::path(const tripoint_om_omt& p) -> bool& {
    if (!inbounds(p)) {
        nullbool = false;
        return nullbool;
    }
...
auto overmap::is_path(const tripoint_om_omt& p) const -> bool {
    if (!inbounds(p)) { return false; }
    return layer[p.z() + OVERMAP_DEPTH].path[p.x()][p.y()];
}

auto overmap::mongroup_check(const mongroup& candidate) const -> bool {
    const auto matching_range = zg.equal_range(
        project_remain<coords::om>(candidate.abs_pos).remainder_tripoint);
    return std::find_if(matching_range.first, matching_range.second,
                        [candidate](const std::pair<tripoint_om_sm, mongroup>& match) {
                            ...
                            return candidate.type == match.second.type
                                && candidate.abs_pos == match.second.abs_pos
...
auto overmap::monster_check(const std::pair<tripoint_om_sm, monster>& candidate) const -> bool {
...
auto overmap::erase_npc(const character_id& id) -> shared_ptr_fast<npc> {
    const auto iter = std::find_if(npcs.begin(), npcs.end(), [id](const shared_ptr_fast<npc>& n) {
        return n->getID() == id;
    });
    if (iter == npcs.end()) { return nullptr; }
    auto ptr = *iter;
    npcs.erase(iter);
    g->set_npcs_dirty();
    return ptr;
...
auto overmap::get_npcs(const std::function<bool(const npc&)>& predicate) const
    -> std::vector<shared_ptr_fast<npc>> {
    std::vector<shared_ptr_fast<npc>> result;
    for (const auto& g : npcs) {
        if (predicate(*g)) { result.push_back(g); }
    }
    return result;
}

auto overmap::has_note(const tripoint_om_omt& p) const -> bool {
    if (p.z() < -OVERMAP_DEPTH || p.z() > OVERMAP_HEIGHT) { return false; }

    for (const om_note& i : layer[p.z() + OVERMAP_DEPTH].notes) {
        if (i.p == p.xy()) { return true; }
    }
    return false;
}

auto overmap::has_note_with_danger_radius(const tripoint_om_omt& p) const -> std::optional<int> {
    if (p.z() < -OVERMAP_DEPTH || p.z() > OVERMAP_HEIGHT) { return std::nullopt; }
...
auto overmap::is_marked_dangerous(const tripoint_om_omt& p) const -> bool {
    for (const om_note& i : layer[p.z() + OVERMAP_DEPTH].notes) {
        if (!i.dangerous) {
            continue;
        } else if (p.xy() == i.p) {
...
auto overmap::all_notes(int z) const -> const std::vector<om_note>& {
    static const std::vector<om_note> fallback;

    if (z < -OVERMAP_DEPTH || z > OVERMAP_HEIGHT) { return fallback; }

    return layer[z + OVERMAP_DEPTH].notes;
}

auto overmap::note(const tripoint_om_omt& p) const -> const std::string& {
...
void overmap::delete_note(const tripoint_om_omt& p) { add_note(p, std::string{}); }

auto overmap::find_notes(const int z, const std::string& text) -> std::vector<point_abs_omt> {
...
auto overmap::has_extra(const tripoint_om_omt& p) const -> bool {
    if (p.z() < -OVERMAP_DEPTH || p.z() > OVERMAP_HEIGHT) { return false; }
...
auto overmap::extra(const tripoint_om_omt& p) const -> const string_id<map_extra>& {
    static const string_id<map_extra> fallback{};

    if (p.z() < -OVERMAP_DEPTH || p.z() > OVERMAP_HEIGHT) { return fallback; }

    const auto& extras = layer[p.z() + OVERMAP_DEPTH].extras;
    const auto it = std::find_if(begin(extras), end(extras), [&](const om_map_extra& n) {
        return n.p == p.xy();
    });
...
auto overmap::find_extras(const int z, const std::string& text) -> std::vector<point_abs_omt> {
...
auto overmap::inbounds(const tripoint_om_omt& p, int clearance) -> bool {
    ...
    static constexpr half_open_cuboid<tripoint_om_omt>
        overmap_boundaries(overmap_boundary_min, overmap_boundary_max);
...
auto overmap::scent_at(const tripoint_abs_omt& loc) const -> const scent_trace& {
    static const scent_trace null_scent;
    const auto& scent_found = scents.find(loc);
    if (scent_found != scents.end()) { return scent_found->second; }
    return null_scent;
}
...
void overmap::generate(
    ...
    if (const dimension_info* dim = g->get_current_dimension_info()) {
        ...
        if (dim->pocket_info.has_value()) {
            dbg(DL::Info) << "overmap::generate skipped for bounded dimension '" << dim->id.str()
                          << "'";
            return;
        }
        if (dim->world_type.is_valid() && !dim->world_type.obj().generate_overmap) {
            dbg(DL::Info) << "overmap::generate skipped for world_type '" << dim->world_type.str()
    ...
    if (settings->overmap_lake.noise_threshold_lake > 0.0) { place_lakes(); }
    if (settings->overmap_forest.noise_threshold_forest > 0.0) {
        place_forests();
        place_swamps();
    }
    place_cities();
    if (settings->forest_trail.chance > 0) { place_forest_trails(); }
    ...
    place_lake_columns();
    if (settings->forest_trail.chance > 0) { place_forest_trailheads(); }
...
static void elevate_bridges(
    overmap& om, const std::vector<point_om_omt>& bridge_points,
    const std::string& bridge_overpass_id, const std::string& bridge_under_id,
    const std::string& bridgehead_ground_id, const std::string& bridgehead_ramp_id,
    const std::string& bridge_center_id, const std::string& bridge_flat_ew_id,
    ...
    for (const point_om_omt& bp : bridge_points) {
        ...
        if (dir == om_direction::type::invalid) {
            // Shouldn't happen
            debugmsg("Potential bridgehead %s at %s has invalid rotation.", ot_here.id(),
                     bp_om.to_string());
            continue;
        }
    ...
    // Put bridge points
    for (const point_om_omt& bp : bridge_points) {
        if (flatten_points.contains(bp)) { continue; }
        tripoint_om_omt p(bp, 0);
    ...
    if (!bridge_center_id.empty()) {
        ...
        om.ter_set(p, oter_id(bridge_center_id
                              + om_direction::all_suffixes[static_cast<int>(center_point.second)]));
    }
}

auto overmap::find_terrain(const std::string& term, int zlevel) -> std::vector<point_abs_omt> {
    std::vector<point_abs_omt> found;
    for (int x = 0; x < OMAPX; x++) {
        for (int y = 0; y < OMAPY; y++) {
            tripoint_om_omt p(x, y, zlevel);
            if (seen(p) && lcmatch(ter(p)->get_name(), term)) {
...
auto overmap::get_nearest_city(const tripoint_om_omt& p) const -> const city& {
    int distance = 999;
...
auto overmap::find_random_omt(const std::pair<std::string, ot_match_type>& target) const
    -> tripoint_om_omt {
    ...
    for (int i = 0; i < OMAPX; i++) {
        for (int j = 0; j < OMAPY; j++) {
            for (int k = -OVERMAP_DEPTH; k <= OVERMAP_HEIGHT; k++) {
                ...
                if (is_ot_match(target.first, ter(p), target.second)) { valid.push_back(p); }
            }
        }
    }
...
void overmap::clear_mon_groups() { zg.clear(); }

void overmap::clear_overmap_special_placements() { overmap_special_placements.clear(); }
void overmap::clear_cities() { cities.clear(); }
void overmap::clear_connections_out() { connections_out.clear(); }
...
void overmap::reset_oter_id_migrations() { oter_id_migrations.clear(); }

auto overmap::is_oter_id_obsolete(const std::string& oterid) -> bool {
...
void overmap::migrate_oter_ids(const std::unordered_map<tripoint_om_omt, std::string>& points) {
    for (const auto& [pos, old_id] : points) {
        ...
        if (new_id.is_valid()) {
            DebugLog(DL::Warn, DC::Map)
                << "migrated oter_id '" << old_id << "' at " << pos_abs << " to '" << new_id.str()
                << "'";

            ter_set(pos, new_id);
        } else {
            debugmsg("oter_id migration defined from '%s' to invalid ter_id '%s'", old_id,
                     new_id.str());
        }
    }
}

void overmap::place_special_forced(
    const overmap_special_id& special_id, const tripoint_om_omt& p, om_direction::type dir) {
    static city invalid_city;
...
void mongroup::wander(const overmap& om) {
    ...
    if (horde_behaviour == "city") {
        ...
        for (const city& check_city : om.cities) {
            ...
            int distance = rl_dist(project_to<coords::sm>(check_city.pos), local_pos.xy());
    ...
    if (target_city) {
        ...
        const auto local_target = tripoint_om_sm(
            ...
            city_sm.y() + rng(-target_city->size * 2, target_city->size * 2), local_pos.z());
        target = project_combine(om.pos(), local_target);
        interest = 100;
    } else {
        const auto local_target = tripoint_om_sm(
            local_pos.x() + rng(-10, 10), local_pos.y() + rng(-10, 10), local_pos.z());
        target = project_combine(om.pos(), local_target);
        interest = 30;
    }
}
...
void overmap::move_hordes() {
    ...
    for (auto it = zg.begin(); it != zg.end();) {
        ...
        if (mg.horde_behaviour.empty()) { mg.horde_behaviour = one_in(2) ? "city" : "roam"; }
        ...
        if ((mg.abs_pos.xy() == mg.target.xy()) || mg.interest <= 15) {
            ...
            if (auto* state = DynamicDataLoader::get_instance().lua.get()) {
                ...
                if (behaviours_obj.is<sol::table>()) {
                    ...
                    const auto fn_obj =
                        behaviours.get_or<sol::object>(mg.horde_behaviour, sol::lua_nil);
                    if (fn_obj.is<sol::protected_function>() || fn_obj.is<sol::function>()) {
                        ...
                        const auto hook_target = results.get<sol::optional<tripoint_abs_sm>>(
                            "target");
                        ...
                        if (hook_interest.has_value()) { mg.set_interest(*hook_interest); }
                    }
                }
            }

            if (!used_hook_target) { mg.wander(*this); }
        }
        ...
        if (one_in(movement_chance) && rng(0, 100) < mg.interest && rng(0, 200) < mg.avg_speed()) {
            if (mg.abs_pos.x() > mg.target.x()) { mg.abs_pos.x()--; }
            if (mg.abs_pos.x() < mg.target.x()) { mg.abs_pos.x()++; }
            if (mg.abs_pos.y() > mg.target.y()) { mg.abs_pos.y()--; }
            if (mg.abs_pos.y() < mg.target.y()) { mg.abs_pos.y()++; }
    ...
    if (get_option<bool>("WANDER_SPAWNS")) {
        ...
        while (monster_map_it != monster_map->end()) {
            ...
            if (!type.species.contains(ZOMBIE) ||         // Only add zombies to hordes.
                type.id == mtype_id("mon_jabberwock") ||  // Jabberwockies are an exception.
                this_monster.get_speed() <= 30 ||         // So are very slow zombies, like crawling
                                                          // zombies.
                ...
                !this_monster.will_join_horde(INT_MAX) || // So are zombies who won't join a horde
                                                          // of any size.
                this_monster.mission_id != -1 // We mustn't delete monsters that are related to
                                              // missions.
            ) {
            ...
            std::for_each(
                group_bucket.first, group_bucket.second,
            ...
            // Check again if the zombie will join the largest horde, now that we know the accurate
            // size.
            ...
            } else { // Bad luck--the zombie would have joined a larger horde, but not this one.
                     // Skip.
                // Don't delete the monster, just increment the iterator.
                monster_map_it++;
                continue;
...
void overmap::move_nemesis() {
    ...
    for (std::multimap<tripoint_om_sm, mongroup>::iterator it = zg.begin(); it != zg.end();) {
        ...
        if (one_in(movement_chance) && rng(0, 200) < mg.avg_speed()) {
            if (mg.abs_pos.x() > mg.nemesis_target.x()) { mg.abs_pos.x()--; }
            if (mg.abs_pos.x() < mg.nemesis_target.x()) { mg.abs_pos.x()++; }
            if (mg.abs_pos.y() > mg.nemesis_target.y()) { mg.abs_pos.y()--; }
            if (mg.abs_pos.y() < mg.nemesis_target.y()) { mg.abs_pos.y()++; }
...
auto overmap::remove_nemesis() -> bool {
    for (std::multimap<tripoint_om_sm, mongroup>::iterator it = zg.begin(); it != zg.end();) {
        mongroup& mg = it->second;
...
void overmap::signal_hordes(const tripoint_abs_sm& p, const int sig_power) {
    for (auto& elem : zg) {
        ...
        const int calculated_inter = (sig_power + 1 - dist) * inter_per_sig_power; // Calculated
                                                                                   // interest
        const int roll = rng(0, mg.interest);
        // Minimum capped calculated interest. Used to give horde enough interest to really
        // investigate the target at start.
...
void overmap::populate_connections_out_from_neighbors(
    const overmap* north, const overmap* east, const overmap* south, const overmap* west) {
            ...
            for (const std::pair<const overmap_connection_id, std::vector<tripoint_om_omt>>& kv :
                ...
                if (adjacent_out != adjacent->connections_out.end()) {
                    for (const tripoint_om_omt& p : adjacent_out->second) {
                        if (should_include(p)) { out.push_back(build_point(p)); }
                    }
                }
            }
        };
    ...
    populate_for_side(
        north, [](const tripoint_om_omt& p) { return p.y() == OMAPY - 1; },
        [](const tripoint_om_omt& p) { return tripoint_om_omt(p.x(), 0, p.z()); });
    populate_for_side(
        west, [](const tripoint_om_omt& p) { return p.x() == OMAPX - 1; },
        [](const tripoint_om_omt& p) { return tripoint_om_omt(0, p.y(), p.z()); });
    populate_for_side(
        south, [](const tripoint_om_omt& p) { return p.y() == 0; },
        [](const tripoint_om_omt& p) { return tripoint_om_omt(p.x(), OMAPY - 1, p.z()); });
    populate_for_side(
        east, [](const tripoint_om_omt& p) { return p.x() == 0; },
        [](const tripoint_om_omt& p) { return tripoint_om_omt(OMAPX - 1, p.y(), p.z()); });
}
...
void overmap::place_forest_trails() {
    ...
    for (int i = 0; i < OMAPX; i++) {
        for (int j = 0; j < OMAPY; j++) {
            ...
            if (visited.contains(seed_point.xy())) { continue; }
            ...
            if (!one_in(forest_trail.chance)) { continue; }

            // Get the north and south most points in the forest.
            auto north_south_most = std::minmax_element(
            ...
            point_om_omt
                center(westmost.x() + (eastmost.x() - westmost.x()) / 2,
            ...
            point_om_omt actual_center_point = *std::min_element(
                forest_points.begin(), forest_points.end(),
                [&center_point](const point_om_omt& lhs, const point_om_omt& rhs) {
                    return square_dist(lhs, center_point) < square_dist(rhs, center_point);
                });
...
void overmap::place_forest_trailheads() {
    ...
    const auto trailhead_close_to_road = [&](const tripoint_om_omt& trailhead) {
        bool close = false;
        for (const tripoint_om_omt& nearby_point :
             closest_points_first(trailhead, settings->forest_trail.trailhead_road_distance)) {
            if (check_ot("road", ot_match_type::contains, nearby_point)) { close = true; }
        }
        return close;
    };

    const auto try_place_trailhead_special =
        [&](const tripoint_om_omt& trail_end, const om_direction::type& dir) {
...
void overmap::place_forests() {
    ...
    for (int x = 0; x < OMAPX; x++) {
        for (int y = 0; y < OMAPY; y++) {
            ...
            if (oter != default_oter_id) { continue; }
...
void overmap::place_lakes() {
    ...
    for (int i = 0; i < OMAPX; i++) {
        for (int j = 0; j < OMAPY; j++) {
            ...
            if (visited.contains(seed_point)) { continue; }

            // It's a lake if it exceeds the noise threshold defined in the region settings.
            if (!is_lake(seed_point)) { continue; }

            // We're going to flood-fill our lake so that we can consider the entire lake when
            // evaluating it for placement, even when the lake runs off the edge of the current
            // overmap.
            std::vector<point_om_omt> lake_points =
                ff::point_flood_fill_4_connected(seed_point, visited, is_lake);

            // If this lake doesn't exceed our minimum size threshold, then skip it. We can use this
            // to exclude the tiny lakes that don't provide interesting map features and exist
            // mostly as a noise artifact.
            ...
            for (auto& p : lake_points) { lake_set.emplace(p); }

            for (int x = 0; x < OMAPX; x++) {
                for (int y = 0; y < OMAPY; y++) {
                    const tripoint_om_omt p(x, y, 0);
                    if (ter(p)->is_river()) { lake_set.emplace(p.xy()); }
                }
            }

            // Iterate through all of our lake points, rejecting the ones that are out of bounds.
            // For those that are inbounds, look at the 8 adjacent locations and see if they are
            // also part of our lake points set. If they are, that means that this location is
            // entirely surrounded by lake and should be considered a lake surface. If at least one
            // adjacent location is not part of this lake points set, that means this location
            // should be considered a lake shore. Either way, make the determination and set the
            // overmap terrain.
            for (auto& p : lake_points) {
                ...
                for (int ni = -1; ni <= 1 && !shore; ni++) {
                    for (int nj = -1; nj <= 1 && !shore; nj++) {
                        ...
                        if (!lake_set.contains(n)) { shore = true; }
                    }
                }
            ...
            const auto connect_lake_to_closest_river =
                [&](const point_om_omt& lake_connection_point) {
                    ...
                    if (closest_distance > 0) { place_river(closest_point, lake_connection_point); }
                };

            // Get the north and south most points in our lake.
            auto north_south_most = std::minmax_element(
                lake_points.begin(), lake_points.end(),
                [](const point_om_omt& lhs, const point_om_omt& rhs) { return lhs.y() < rhs.y(); });

            point_om_omt northmost = *north_south_most.first;
            point_om_omt southmost = *north_south_most.second;

            // It's possible that our northmost/southmost points in the lake are not on this
            // overmap, because our lake may extend across multiple overmaps.
            if (inbounds(northmost)) { connect_lake_to_closest_river(northmost); }

            if (inbounds(southmost)) { connect_lake_to_closest_river(southmost); }
        }
    }
}

void overmap::place_rivers(
    const overmap* north, const overmap* east, const overmap* south, const overmap* west) {
    if (settings->river_scale == 0.0) { return; }
    ...
    if (north != nullptr) {
        for (int i = 2; i < OMAPX - 2; i++) {
            ...
            if (is_river(north->ter(p_neighbour))) { ter_set(p_mine, river_center); }
            if (is_river(north->ter(p_neighbour)) && is_river(north->ter(p_neighbour + point_east))
                && is_river(north->ter(p_neighbour + point_west))) {
                if (one_in(river_chance)
                    && (river_start.empty()
    ...
    if (west != nullptr) {
        for (int i = 2; i < OMAPY - 2; i++) {
            ...
            if (is_river(west->ter(p_neighbour))) { ter_set(p_mine, river_center); }
            if (is_river(west->ter(p_neighbour)) && is_river(west->ter(p_neighbour + point_north))
                && is_river(west->ter(p_neighbour + point_south))) {
                if (one_in(river_chance)
                    && (river_start.size() == rivers_from_north
    ...
    if (south != nullptr) {
        for (int i = 2; i < OMAPX - 2; i++) {
            ...
            if (is_river(south->ter(p_neighbour))) { ter_set(p_mine, river_center); }
            if (is_river(south->ter(p_neighbour)) && is_river(south->ter(p_neighbour + point_east))
                && is_river(south->ter(p_neighbour + point_west))) {
                if (river_end.empty() || river_end[river_end.size() - 1].x() < i - 6) {
    ...
    if (east != nullptr) {
        for (int i = 2; i < OMAPY - 2; i++) {
            ...
            if (is_river(east->ter(p_neighbour))) { ter_set(p_mine, river_center); }
            if (is_river(east->ter(p_neighbour)) && is_river(east->ter(p_neighbour + point_north))
    ...
    if (river_start.size() > river_end.size() && !river_end.empty()) {
        ...
        if (river_start.size() != river_end.size()) {
            river_start
                .emplace_back(rng(OMAPX / 4, (OMAPX * 3) / 4), rng(OMAPY / 4, (OMAPY * 3) / 4));
        }
...
auto overmap::place_lake_columns() -> void {
    ...
    if (surface_column.empty() && shore_column.empty()) { return; }

    const auto place_column =
        [this](const tripoint_om_omt& surface, const std::vector<oter_str_id>& column) {
            using namespace std::views;

            for (const auto offset : iota(0, static_cast<int>(column.size()))) {
                const auto z = -offset - 1;
                if (z < -OVERMAP_DEPTH) { break; }
...
void overmap::place_swamps() {
    ...
    for (int x = 0; x < OMAPX; x++) {
        for (int y = 0; y < OMAPY; y++) {
            ...
            if (is_ot_match("river", ter(pos), ot_match_type::contains)) {
                std::vector<point_om_omt> buffered_points = closest_points_first(
                    pos.xy(),
    ...
    for (int x = 0; x < OMAPX; x++) {
        for (int y = 0; y < OMAPY; y++) {
            const tripoint_om_omt pos(x, y, 0);
            // If this location isn't a forest, there's nothing to do here. We'll only grow swamps
            // in existing forest terrain.
            if (!is_ot_match("forest", ter(pos), ot_match_type::contains)) { continue; }

            // If this was a part of our buffered floodplain, and the noise here meets the
            // threshold, and the one_in rng triggers, then we should flood this location and make
            // it a swamp.
            const bool should_flood =
                (floodplain[x][y] > 0 && !one_in(floodplain[x][y])
                 && f.noise_at({x, y})
                        > settings->overmap_forest.noise_threshold_swamp_adjacent_water);

            // If this location meets our isolated swamp threshold, regardless of floodplain values,
            // we'll make it into a swamp.
            const bool should_isolated_swamp =
                f.noise_at(pos.xy()) > settings->overmap_forest.noise_threshold_swamp_isolated;
            if (should_flood || should_isolated_swamp) { ter_set(pos, forest_water); }
        }
    }
}
...
void overmap::place_roads(
    ...
    if (roads_out.size() < 2) {
        ...
        for (int i = 0; i < 160; i++) { omap_num[i] = i + 10; }
    ...
    for (const auto& elem : roads_out) { road_points.emplace_back(elem.xy()); }
    for (const auto& elem : cities) { road_points.emplace_back(elem.pos); }
...
void overmap::place_river(point_om_omt pa, point_om_omt pb) {
    ...
    do {
        ...
        p2.y() += rng(-1, 1);
        if (p2.x() < 0) { p2.x() = 0; }
        if (p2.x() > OMAPX - 1) { p2.x() = OMAPX - 1; }
        if (p2.y() < 0) { p2.y() = 0; }
        if (p2.y() > OMAPY - 1) { p2.y() = OMAPY - 1; }
        for (int i = -1 * river_scale; i <= 1 * river_scale; i++) {
            for (int j = -1 * river_scale; j <= 1 * river_scale; j++) {
                ...
                if (p.y() >= 0 && p.y() < OMAPY && p.x() >= 0 && p.x() < OMAPX) {
                    if (!ter(p)->is_lake() && one_in(river_chance)) { ter_set(p, river_center); }
                }
            }
        }
        if (pb.x() > p2.x()
            && (rng(0, static_cast<int>(OMAPX * 1.2) - 1) < pb.x() - p2.x()
                || (rng(0, static_cast<int>(OMAPX * 0.2) - 1) > pb.x() - p2.x()
                    && rng(0, static_cast<int>(OMAPY * 0.2) - 1) > std::abs(pb.y() - p2.y())))) {
            p2.x()++;
        }
        if (pb.x() < p2.x()
            && (rng(0, static_cast<int>(OMAPX * 1.2) - 1) < p2.x() - pb.x()
                || (rng(0, static_cast<int>(OMAPX * 0.2) - 1) > p2.x() - pb.x()
                    && rng(0, static_cast<int>(OMAPY * 0.2) - 1) > std::abs(pb.y() - p2.y())))) {
            p2.x()--;
        }
        if (pb.y() > p2.y()
            && (rng(0, static_cast<int>(OMAPY * 1.2) - 1) < pb.y() - p2.y()
                || (rng(0, static_cast<int>(OMAPY * 0.2) - 1) > pb.y() - p2.y()
                    && rng(0, static_cast<int>(OMAPX * 0.2) - 1) > std::abs(p2.x() - pb.x())))) {
            p2.y()++;
        }
        if (pb.y() < p2.y()
            && (rng(0, static_cast<int>(OMAPY * 1.2) - 1) < p2.y() - pb.y()
...
void overmap::place_cities() {
    ...
    int op_city_size =
        city_spec.city_size >= 0 ? city_spec.city_size : get_option<int>("CITY_SIZE");
    if (op_city_size <= 0) { return; }
    int op_city_spacing =
        city_spec.city_spacing >= 0 ? city_spec.city_spacing : get_option<int>("CITY_SPACING");
    ...
    const int NUM_CITIES = roll_remainder(
    ...
    // if there is only a single free tile, the probability of NOT finding it after
    // MAX_PLACEMENT_ATTEMTPS attempts is (1 - 1/(OMAPX * OMAPY))^MAX_PLACEMENT_ATTEMTPS = approx.
    // 36% for the OMAPX=OMAPY=180 and MAX_PLACEMENT_ATTEMTPS=OMAPX * OMAPY
    ...
    while (cities.size() < static_cast<size_t>(NUM_CITIES)
        ...
        if (!city_spec.finales.finalized) { tmp.attempt_finale = false; }
        size = std::max(size, 1);
        ...
        // attempt to generate a city with a finale if it's not tiny. If it's tiny just run once via
        // a do while.
        do {
            ...
            if (ter(p) == settings->default_oter) {
                ...
                // if the city finale failed to place, restore from last backup and try again at the
                // top of the loop
            ...
            finale_attempts++;
        } while ((tmp.attempt_finale && !tmp.finale_placed && finale_attempts < finale_max_tries));
    }
}
...
auto overmap::place_isolated_cities() -> void {
    ...
    const auto configured_city_size =
        config.city_size >= 0 ? config.city_size : get_option<int>("CITY_SIZE");
    const auto maximum_city_size =
        config.max_city_size >= 0 ? config.max_city_size : std::numeric_limits<int>::max();
    const auto city_size = std::min(configured_city_size, maximum_city_size);
    if (city_size <= 0 || maximum_city_size < config.min_city_size) { return; }

    const auto city_spacing = std::
        max(config.city_spacing >= 0 ? config.city_spacing : get_option<int>("CITY_SPACING"), 0);

    const auto base_oter_str =
        config.base_oter.str().empty() ? settings->default_oter : config.base_oter;
    const auto has_shore_oter = !config.shore_oter.str().empty();
    if (!base_oter_str.is_valid() || !config.land_oter.is_valid()
        || (has_shore_oter && !config.shore_oter.is_valid()) || !config.center_oter.is_valid()
        || !config.road_connection.is_valid()
        || !ranges::all_of(
            config.required_specials, [](const auto& anchor) { return anchor.special.is_valid(); })
        || !ranges::all_of(config.optional_specials, [](const auto& anchor) {
               return anchor.special.is_valid();
           })) {
        debugmsg(
            "Invalid isolated_city settings for region %s: base_oter=%s land_oter=%s "
            "shore_oter=%s center_oter=%s road_connection=%s",
            settings->id.c_str(), base_oter_str.c_str(), config.land_oter.c_str(),
            config.shore_oter.c_str(), config.center_oter.c_str(), config.road_connection.c_str());
        return;
    }
    ...
    const auto shore_oter =
        has_shore_oter ? std::optional<oter_id>(oter_id(config.shore_oter)) : std::nullopt;
    ...
    const auto num_cities = roll_remainder(
        omts_per_overmap * city_map_coverage_ratio / omts_per_city);

    const auto max_radius = std::min(OMAPX, OMAPY) / 2 - 2;
    if (max_radius <= 0) { return; }

    const auto find_special_placement = [this](const isolated_city_special_placement_options& opts)
        ...
        if (opts.placement == "shore") {
            ...
            using namespace std::views;
            for (const auto x :
                 iota(opts.center.x() - outer_edge, opts.center.x() + outer_edge + 1)) {
                for (const auto y :
                     iota(opts.center.y() - outer_edge, opts.center.y() + outer_edge + 1)) {
                    const auto candidate = tripoint_om_omt(x, y, 0);
                    const auto distance = trig_dist(candidate.xy(), opts.center.xy());
                    if (distance <= outer_edge) { candidates.push_back(candidate); }
                }
            }
        } else {
    ...
    const auto anchor_applies =
        [](const isolated_city_special_settings& anchor, const int size, const int island_radius) {
    ...
    const auto trim_island_to_city_shape =
        [this, &base_oter, &land_oter, &shore_oter,
         &road_connection](const isolated_city_trim_options& opts) {
            ...
            for (const auto& tile : opts.island_tiles) {
                if (opts.shore_anchor_tiles.contains(tile.pos)) { continue; }
            ...
            if (structure_tiles.empty() && road_tiles.empty()) { return; }

            if (structure_tiles.empty()) { structure_tiles = road_tiles; }

            const auto land_buffer = opts.land_padding;
            const auto road_buffer = opts.road_padding;
            const auto protected_structure_distance =
                land_buffer > 0
                    ? std::max(1, land_buffer - std::max(2, opts.coastline_variance / 3))
                    : 0;
            const auto protected_road_distance = road_buffer > 0 ? std::max(1, road_buffer - 1) : 0;
            ...
            for (const auto& tile : opts.island_tiles) {
                ...
                const auto structure_distance =
                    isolated_city_nearest_development_distance(tile.pos, structure_tiles);
                const auto road_distance =
                    isolated_city_nearest_development_distance(tile.pos, road_tiles);
                const auto coastline_adjustment = isolated_city_coastline_adjustment(
                    opts.center, tile.pos, opts.coastline_variance);
                const auto near_structure =
                    structure_distance.has_value()
                    && *structure_distance <= std::max(
                           protected_structure_distance, land_buffer + coastline_adjustment);
                const auto protected_structure =
                    structure_distance.has_value()
                    && *structure_distance <= protected_structure_distance;
                const auto protected_road =
                    road_distance.has_value() && *road_distance <= protected_road_distance;
                const auto road_adjustment = coastline_adjustment / 2;
                const auto near_adjusted_road =
                    road_distance.has_value()
                    && *road_distance
                           <= std::max(protected_road_distance, road_buffer + road_adjustment);
            ...
            const auto protected_retained_land =
                [&structure_tiles, &road_tiles, protected_structure_distance,
                 protected_road_distance](const tripoint_om_omt& pos) {
                    const auto structure_distance =
                        isolated_city_nearest_development_distance(pos, structure_tiles);
                    const auto road_distance =
                        isolated_city_nearest_development_distance(pos, road_tiles);
            ...
            for (const auto pass : iota(0, 2)) {
                ...
                for (const auto& tile : opts.island_tiles) {
                    ...
                    if (neighbor_count >= 8 || protected_retained_land(tile.pos)) { continue; }
            ...
            for (const auto& tile : solid_tiles) { clear_submerged_lake_column(tile); }

            if (!shore_oter.has_value() || opts.shore_width <= 0) { return; }

            const auto set_submerged_lake_shore_column = [this](const tripoint_om_omt& surface) {
                using namespace std::views;

                static const oter_id lake_underwater_shore("lake_underwater_shore");
                const auto shore_depth =
                    std::max(-OVERMAP_DEPTH, settings->overmap_lake.lake_depth);
                for (const auto z : iota(shore_depth, 0)) {
                    const auto column_pos = tripoint_om_omt(surface.xy(), z);
                    const auto& current_oter = ter(column_pos);
                    if (isolated_city_lake_column_or_default_terrain(
                            current_oter, get_default_terrain(z))) {
            ...
            for (const auto& tile : solid_tiles) {
                shore_scan_radius = std::max(
                    shore_scan_radius,
                    static_cast<int>(std::ceil(trig_dist(tile.xy(), opts.center.xy())))
                        + opts.shore_width);
            }
            const auto shore_scan_tiles =
                isolated_city_tiles_in_radius(opts.center, shore_scan_radius);

            for (const auto& tile : shore_scan_tiles) {
                if (!inbounds(tile.pos)) { continue; }
    ...
    const auto connect_anchor_to_city_roads =
        [this,
         &road_connection](const city& town, const std::vector<tripoint_om_omt>& anchor_tiles) {
            ...
            for (const auto& tile : anchor_tiles) {
                if (tile.z() == 0) { anchor_footprint.insert(tile); }
            }
            for (const auto& tile : anchor_footprint) {
                for (const auto dir : om_direction::all) {
                    ...
                    if (candidate_set.insert(candidate).second) { candidates.push_back(candidate); }
                }
            }
            ...
            return ranges::
                any_of(candidates, [this, &town, &road_connection](const auto& candidate) {
                    return build_connection(
                        town.pos, candidate.xy(), candidate.z(), road_connection, false);
                });
        };
    ...
    while (placed < num_cities && placement_attempts < max_placement_attempts) {
        ...
        auto town = city();
        const auto profile =
            random_city_generation_profile(city_size, settings->city_spec.finales.finalized);
        ...
        const auto land_padding =
            config.land_padding >= 0
                ? config.land_padding
                : isolated_city_default_land_padding(size);
        const auto road_padding =
            config.road_padding >= 0
                ? config.road_padding
        ...
        const auto shore_width =
            shore_oter.has_value() ? std::clamp(config.shore_width, 0, island_radius) : 0;
        const auto land_radius = island_radius - shore_width;
        const auto coastline_variance =
            config.coastline_variance >= 0
        ...
        const auto city_generation_radius = std::clamp(
            std::max(island_radius,
                     size * 2 + land_padding + shore_width + std::max(2, coastline_variance / 2)),
        ...
        const auto center = tripoint_om_omt(
            rng(city_generation_radius + 1, OMAPX - city_generation_radius - 2),
            rng(city_generation_radius + 1, OMAPY - city_generation_radius - 2), 0);
        ...
        const auto placement_land_radius =
            shore_oter.has_value() ? city_generation_radius - 1 : city_generation_radius;
        for (const auto& tile : island_tiles) {
            const auto tile_oter =
                shore_oter.has_value() && tile.distance > placement_land_radius
                    ? *shore_oter
                    : land_oter;
        ...
        if (center_oter == road_manhole) { ter_set(center + tripoint_below, sewer_isolated); }
        ...
        const auto place_anchor = [&](const isolated_city_special_settings& anchor) {
            if (!anchor_applies(anchor, size, island_radius)) { return true; }
            const auto& special = *anchor.special;
            const auto placement_radius =
                anchor.placement == "shore" ? island_radius : city_shape_radius;
            ...
            const auto placed_tiles =
                place_special(special, placement->pos, placement->dir, town, false, false);
            anchor_placements.push_back(placed_tiles);
            if (anchor.placement == "shore") {
                for (const auto& placed_tile : placed_tiles) {
                    if (placed_tile.z() == 0) { shore_anchor_tiles.insert(placed_tile); }
                }
            }
            return true;
        };
        ...
        for (const auto& anchor : config.optional_specials) {
            if (!anchor_applies(anchor, size, island_radius) || anchor.chance <= 0) { continue; }
            if (anchor.placement == "shore") {
                delayed_optional_anchors.push_back(anchor);
                continue;
            }
            if (anchor.chance >= 100 || x_in_y(anchor.chance, 100)) { place_anchor(anchor); }
        }
        ...
        if (config.register_city) { cities.push_back(town); }
        placed += 1;
        placement_attempts = 0;
    }
    ...
    if (num_cities > 0 && placed == 0) {
        dbg(DL::Info) << "isolated city placement failed for region " << settings->id
                      << ": wanted=" << num_cities << " attempts=" << placement_attempts
                      << " skipped_small=" << skipped_small << " skipped_center=" << skipped_center
...
auto overmap::pick_random_building_to_place(
    ...
    int shop_normal = std::max(static_cast<int>(normal_roll(shop_radius, shop_sigma)), shop_radius);
    int apartment_normal = std::
        max(static_cast<int>(normal_roll(apartment_radius, apartment_sigma)), apartment_radius);
    int park_normal = std::max(static_cast<int>(normal_roll(park_radius, park_sigma)), park_radius);
...
auto overmap::place_building(
    ...
    for (size_t retries = 10; retries > 0; --retries) {
        const overmap_special_id building_tid =
            pick_random_building_to_place(town_dist, town.size, attempt_finale_place);

        if (!building_tid.is_valid()) { return false; }
...
void overmap::build_city_street(
    ...
    city& town, std::vector<tripoint_om_omt>& sewers, int block_width) {
    int c = cs;
    int croad = cs;
    ...
    for (auto iter = from; iter != to; ++iter) {
        ...
        if (c >= 2 && c < croad - block_width) {
            ...
            build_city_street(
                connection, iter->pos, left, om_direction::turn_left(dir), town, sewers, new_width);

            build_city_street(connection, iter->pos, right, om_direction::turn_right(dir), town,
                              sewers, new_width);
        ...
        if (!one_in(BUILDINGCHANCE)) {
            if (attempt_finale_place && !town.finale_placed) {
                ...
                } else { // if the finale fails to place stop trying. This prevents the finale
                         // getting placed at the edge of town.
...
auto overmap::lay_out_connection(
    const overmap_connection& connection, const point_om_omt& source, const point_om_omt& dest,
    int z, const bool must_be_unexplored) const -> pf::directed_path<point_om_omt> {
    const pf::two_node_scoring_fn<point_om_omt> estimate =
        [&](pf::directed_node<point_om_omt> cur,
            ...
            if (must_be_unexplored && !existing_connection) {
                ...
                if (existing_submap) { return pf::node_score::rejected; }
            }
            ...
            if (prev && prev->dir != om_direction::type::invalid && prev->dir != cur.dir) {
                ...
                const overmap_connection::subtype* prev_subtype = connection.pick_subtype_for(
                    prev_id);
            ...
            if (prev && prev->dir == om_direction::type::invalid
                && cur.dir != om_direction::type::invalid) {
                // Non-linear connections starting near overmap border should always be
                // perpendicular to that border
                const oter_id& prev_id = ter(tripoint_om_omt(prev->pos, z));

                if (!connection.can_start_at(prev_id) && !inbounds(cur.pos, 1)
                    && inbounds(
                        prev->pos + om_direction::displace(om_direction::opposite(cur.dir), 1))) {
                    return pf::node_score::rejected;
                }
            }

            int score =
                subtype->is_orthogonal() ? manhattan_dist(dest, cur.pos) : trig_dist(dest, cur.pos);
            if (!existing_connection) {
                // Prefer existing connections
                score *= 5;
            }
            if (!inbounds(cur.pos, 1)) {
                // Roads running next to overmap edge often looks unnatural and weird, better avoid
                // them
                score *= 2;
            }
...
static auto straight_path(const point_om_omt& source, om_direction::type dir, size_t len)
    -> pf::directed_path<point_om_omt> {
...
auto overmap::lay_out_street(
    ...
    if (inbounds(en_pos, 1) && connection.has(ter(en_pos))) { ++len; }
    ...
    while (actual_len < len) {
        ...
        if (ter_id->is_river() || !connection.pick_subtype_for(ter_id)) { break; }
        ...
        for (int i = -1; i <= 1; i++) {
            ...
            for (int j = -1; j <= 1; j++) {
                ...
                if (checkp != pos + om_direction::displace(dir, 1)
                    ...
                    && checkp != pos) {
                    if (is_ot_match("road", ter(checkp), ot_match_type::type)) { collisions++; }
                }
            }
...
auto overmap_connection_cache::get_all(const overmap_connection_id& id, const int z)
    -> const std::vector<point_om_omt>& {
...
auto overmap_connection_cache::get_closests(
...
void overmap_connection_cache::add(
    const overmap_connection_id& id, const int z, const point_om_omt& pos) {
    auto outer = cache.find(id);
    if (outer == cache.end()) {
...
auto overmap::build_connection(
    ...
    for (const auto& node : path.nodes) {
        ...
        if (!subtype) {
            debugmsg("No suitable subtype of connection \"%s\" found for \"%s\".",
                     connection.id.c_str(), ter_id.id().c_str());
            return false;
        }
                ...
                if (inbounds(np)) {
                    ...
                    if (connection.has(near_id)) {
                        if (near_id->is_linear()) {
                            ...
                            if (om_lines::is_straight(near_line)
                                || om_lines::has_segment(near_line, new_dir)) {
                                // Mutual connection.
                                const size_t new_near_line =
                                    om_lines::set_segment(near_line, om_direction::opposite(dir));
                                ter_set(np, near_id->get_type_id()->get_linear(new_near_line));
                                new_line = om_lines::set_segment(new_line, dir);
                            }
                        } else if (near_id->is_rotatable()
                                   && om_direction::are_parallel(dir, near_id->get_dir())) {
                            new_line = om_lines::set_segment(new_line, dir);
                        }
                    }
                } else if (pos.xy() == start.pos || pos.xy() == end.pos) {
                    // Only automatically connect to out of bounds locations if we're the start or
                    // end of this path.
                    new_line = om_lines::set_segment(new_line, dir);

                    // Add this connection point to our connections out.
                    std::vector<tripoint_om_omt>& outs = connections_out[connection.id];
                    const auto existing_out =
                        std::find_if(outs.begin(), outs.end(), [pos](const tripoint_om_omt& c) {
                            return c == pos;
                        });
                    if (existing_out == outs.end()) { outs.emplace_back(pos); }
                }
            }
    ...
    if (connection_cache) { connection_cache->add(connection.id, z, start.pos); }
    ...
    if (z == 0 && connection.id.str() == "local_road") {
        ...
        if (!has_bridge) { return true; }
        if (oter_str_id(name + "_center_under_north").is_valid()) {
            elevate_bridges(
                *this, bridge_points, name + "_road", name + "_under", name + "head_ground",
                name + "head_ramp", name + "_center_under", "road_ew", "road_ns");
        } else {
            elevate_bridges(*this, bridge_points, name + "_road", name + "_under",
                            name + "head_ground", name + "head_ramp", "", "road_ew", "road_ns");
        }
    }
    return true;
}
...
auto overmap::build_connection(
    ...
    const cube_direction initial_dir) -> bool {
    return build_connection(
        connection, lay_out_connection(connection, source, dest, z, must_be_unexplored), z,
        initial_dir);
}

void overmap::connect_closest_points(
...
auto overmap::check_overmap_special_type(
    ...
    if (found_id == overmap_special_placements.end()) { return false; }

    // Return whether the found special was a match with our requested id.
    return found_id->second == id;
}

auto overmap::overmap_special_at(const tripoint_om_omt& p) const
    -> std::optional<overmap_special_id> {
    auto it = overmap_special_placements.find(p);

    if (it == overmap_special_placements.end()) { return std::nullopt; }

    return it->second;
}

void overmap::polish_rivers(
    const overmap* north, const overmap* east, const overmap* south, const overmap* west) {
    for (int x = 0; x < OMAPX; x++) {
        for (int y = 0; y < OMAPY; y++) {
            const tripoint_om_omt p = {x, y, 0};

            if (!is_ot_match("river", ter(p), ot_match_type::prefix)) { continue; }

            // For ungenerated overmaps we're assuming that terra incognita is covered by water, and
            // leaving polishing to its mapgen
            bool water_north =
                y != 0 ? is_river_or_lake(ter({x, y - 1, 0}))
                : north != nullptr
                    ? is_river_or_lake(north->ter({x, OMAPY - 1, 0}))
                    : true;
            bool water_west =
                x != 0 ? is_river_or_lake(ter({x - 1, y, 0}))
                : west != nullptr
                    ? is_river_or_lake(west->ter({OMAPX - 1, y, 0}))
                    : true;
            bool water_south =
                y != OMAPY - 1 ? is_river_or_lake(ter({x, y + 1, 0}))
                : south != nullptr
                    ? is_river_or_lake(south->ter({x, 0, 0}))
                    : true;
            bool water_east =
                x != OMAPX - 1 ? is_river_or_lake(ter({x + 1, y, 0}))
                : east != nullptr
                    ? is_river_or_lake(east->ter({0, y, 0}))
                    : true;
                ...
                if (water_north) {
                    if (water_south) {
                        if (water_east) {
                            ...
                            if ((x > 0 && y > 0 && !is_river_or_lake(ter({x - 1, y - 1, 0})))
...
auto om_direction::name(type dir) -> std::string {
    static const std::array<std::string, size + 1> names = {
        {translate_marker("invalid"), translate_marker("north"), translate_marker("east"),
         translate_marker("south"), translate_marker("west")}};
...
auto om_direction::rotate(point p, type dir) -> point {
    switch (dir) {
        case om_direction::type::invalid:
        case om_direction::type::last:
...
auto om_direction::rotate(const tripoint& p, type dir) -> tripoint {
    return tripoint(rotate({p.xy()}, dir), p.z);
}

auto om_direction::rotate_symbol(uint32_t sym, type dir) -> uint32_t {
    return rotatable_symbols::get(sym, static_cast<int>(dir));
}

auto om_direction::displace(type dir, int dist) -> point { return rotate({0, -dist}, dir); }

inline auto rotate_internal(om_direction::type dir, int step) -> om_direction::type {
    if (dir == om_direction::type::invalid) {
...
auto om_direction::add(type dir1, type dir2) -> om_direction::type {
    return rotate_internal(dir1, static_cast<int>(dir2));
}

auto om_direction::turn_left(type dir) -> om_direction::type {
    return rotate_internal(dir, -static_cast<int>(size) / 4);
}

auto om_direction::turn_right(type dir) -> om_direction::type {
    return rotate_internal(dir, static_cast<int>(size) / 4);
}

auto om_direction::turn_random(type dir) -> om_direction::type {
    return rng(0, 1) ? turn_left(dir) : turn_right(dir);
}

auto om_direction::opposite(type dir) -> om_direction::type {
    return rotate_internal(dir, static_cast<int>(size) / 2);
}

auto om_direction::random() -> om_direction::type { return static_cast<type>(rng(0, size - 1)); }

auto om_direction::are_parallel(type dir1, type dir2) -> bool {
    return dir1 == dir2 || dir1 == opposite(dir2);
}

auto om_direction::from_cube(cube_direction c, const std::string& error_msg) -> om_direction::type {
    switch (c) {
        case cube_direction::north:
...
auto overmap::random_special_rotation(
    const overmap_special& special, const tripoint_om_omt& p, const bool must_be_unexplored) const
    -> om_direction::type {
    ...
    // Try to find the most suitable rotation: satisfy as many connections as possible with the
    // existing terrain.
...
auto overmap::can_place_special(
    ...
    return std::all_of(
        fixed_terrains.begin(), fixed_terrains.end(), [&](const overmap_special_locations& elem) {
            const tripoint_om_omt rp = p + om_direction::rotate(elem.p, dir);

            if (!inbounds(rp, 1)) { return false; }
...
auto overmap::place_special(
    ...
    auto* fluid_connections = fluid_grid_enabled ? &fluid_grid::connections_for(*this) : nullptr;
    ...
    for (const overmap_special_connection& elem : special.connections) {
        ...
        if (initial_dir != cube_direction::last) { initial_dir = initial_dir + dir; }
    ...
    for (const placed_connection& elem : result.cons_used) {
        if (elem.connection) {
            ...
            if (!elem.connection->can_start_at(ter(rp))) { continue; }
            ...
            if (elem.connection->get_layout() == overmap_connection_layout::city && cit) {
                ...
                linked = build_connection(
                    cit.pos, rp.xy(), rp.z(), *elem.connection, must_be_unexplored, initial_dir);
            }
            if (!linked && connection_cache) {
                ...
                for (const point_om_omt& pos : points) {
                    if ((linked = build_connection(
                             pos, rp.xy(), rp.z(), *elem.connection, must_be_unexplored,
                             initial_dir))
                        || ++attempts > 10) {
                        break;
                    }
            ...
            if (!linked && !connection_cache) {
                ...
                for (const tripoint_om_omt& p : closest_points_first(rp, OMAPX)) {
                    if (!inbounds(p)
                        || std::find(result.omts_used.begin(), result.omts_used.end(), p)
                               != result.omts_used.end()
                        || !check_ot(elem.connection->id->default_terrain.str(),
                                     ot_match_type::type, p)) {
                        continue;
                    }
                    if ((linked = build_connection(
                             p.xy(), rp.xy(), rp.z(), *elem.connection, must_be_unexplored,
                             initial_dir))
                        || ++attempts > 10) {
                        break;
                    }
            ...
            if (!linked && initial_dir != cube_direction::last) {
                // If nothing found, make a stub for a clean break, and also to connect other
                // specials here later on
                pf::directed_path<point_om_omt> stub;
                stub.nodes.emplace_back(
                    rp.xy(),
                    om_direction::opposite(om_direction::from_cube(
                        initial_dir, "Up and down connections not yet supported")));
    ...
    for (const tripoint_om_omt& location : result.omts_used) {
        ...
        if (grid || fluid_connections) {
            for (size_t i = 0; i < six_cardinal_directions.size(); i++) {
                ...
                if (std::find(result.omts_used.begin(), result.omts_used.end(), other)
                    != result.omts_used.end()) {
                    if (grid) { electric_grid_connections[location].set(i, true); }
                    if (fluid_connections) { (*fluid_connections)[location].set(i, true); }
                }
            }
        }
    }
    ...
    if (spawns.group) {
        ...
        add_mon_group(
            mongroup(spawns.group, project_combine(pos(), project_to<coords::sm>(p)), rad, pop));
    }
    ...
    for (const auto& nested : special.get_nested_specials()) {
        ...
        if (can_place_special(*nested.second, rp, dir, false)) {
            std::vector<tripoint_om_omt> nested_result =
...
void overmap::spawn_ores(const tripoint_abs_omt& p) {
    ...
    if (one_in(65 - abs(5 * p.z()))) {
        ...
        for (const auto& [k, v] : ore_depth_to_rate.at(depth)) { ores.add(k, v); }
        ...
        if (!(MAPBUFFER_REGISTRY.get(dimension_id_).lookup_submap(target_sub))) {
            ...
            ter_set(local_pos, oter_id("omt_ore_vein_" + chosen + directions[rand() % 4]));
            return;
        }
        ...
        for (const auto offset :
             point_range<point_omt_sm>(point_omt_sm::zero(), point_omt_sm(1, 1))) {
            ...
            if (destsm == nullptr || srcsm == nullptr) { continue; }

            submap::swap(*destsm, *srcsm);

            for (auto& veh : destsm->vehicles) { veh->abs_sm_pos = dest_pos; }
        ...
        // Since we cleared the vehicle cache of the whole z-level (not just the generate map), we
        // add it back here
...
struct specials_overlay {
    ...
    specials_overlay(std::vector<tripoint_om_omt>& points) {
        ...
        order = {std::make_move_iterator(points.begin()), std::make_move_iterator(points.end())};

        for (int x = 0; x < OMAPX; x++) {
            for (int y = 0; y < OMAPY; y++) { lookup[x][y] = order.end(); }
        }
        for (auto it = order.begin(); it != order.end(); it++) { lookup[it->x()][it->y()] = it; }
    }

    auto begin() -> std::list<tripoint_om_omt>::iterator { return order.begin(); }
    auto end() -> std::list<tripoint_om_omt>::iterator { return order.end(); }

    void erase_other(const std::list<tripoint_om_omt>::iterator& i, const tripoint_om_omt& pos) {
        if (!overmap::inbounds(pos) || i->raw() == pos.raw()) { return; }
    ...
    auto erase_this(const std::list<tripoint_om_omt>::iterator& i)
        -> std::list<tripoint_om_omt>::iterator {
...
auto overmap::place_special_custom(
    const overmap_special& special, std::vector<tripoint_om_omt>& points) -> int {
...
auto overmap::place_special_attempt(
    ...
    for (auto p = points.begin(); p != points.end();) {
        ...
        if (need_city) {
            if (!valid_city.contains(&nearest_city) || valid_city[&nearest_city] >= max_per_city
        ...
        if (need_city) { valid_city[&nearest_city]++; }

        // Remove all used points from our candidates list
        if (RANGE == 0) {
            for (const auto& dest : result) {
                if (dest.z() == 0) { points.erase_other(p, dest); }
            }
        } else if (RANGE > 0) {
            ...
            for (const tripoint_om_omt& dest :
                 tripoint_range<tripoint_om_omt>(p_min - shift, p_max + shift)) {
...
void overmap::place_specials(overmap_special_batch& enabled_specials) {
    ...
    enum zone : int { land, land_under, lake, river, last };
    ...
    for (auto& iter : enabled_specials) {
        ...
        zone current =
            special.has_flag("LAKE")    ? zone::lake
        ...
        if (current == zone::land) { land_locs.insert(this_locs.begin(), this_locs.end()); }

        const numeric_interval<int>& o = special.get_constraints().occurrences;

        float average_amount =
            special.has_flag("UNIQUE")
                ? static_cast<float>(o.min) / o.max
                : static_cast<float>(o.min + o.max) / 2;

        special_area[special.id] =
            current == zone::land_under ? this_area.under : this_area.surface;
        float this_range = current == zone::land_under ? 0 : RANGE;
        float total_area =
            std::pow(std::sqrt(special_area[special.id]) + this_range, 2.0) * average_amount
            * DENSITY;
    ...
    const auto special_weight = [&](const overmap_special* s) {
        ...
        if (s->use_absolute_spawn_loc()) {
            if (s->at_absolute_spawn_loc(current_om)) { weight *= 1000; }
            // Make certain global unique specials flagged as specific to endgame don't spawn
            // elsewhere.
    ...
    std::sort(
        enabled_specials.begin(), enabled_specials.end(),
    ...
    for (auto& iter : enabled_specials) {
        ...
        const float rate =
            special.use_absolute_spawn_loc() && special.at_absolute_spawn_loc(current_om)
                ? 1
                : zone_ratio[current];
        ...
        if ((unique || globally_unique)
            ...
            if (special.use_absolute_spawn_loc() && globally_unique) {
            ...
            } else if (special.use_absolute_spawn_loc()
                       && special.at_absolute_spawn_loc(current_om)) {
                amount_to_place = 1;
            } else {
                ...
                amount_to_place =
                    x_in_y(min, max)
                            && (!globally_unique
                                || !get_overmapbuffer(dimension_id_).contains_unique_special(id))
                        ? 1
                        : 0;
            }
        } else {
...
void overmap::place_mongroups() {
            ...
            if (!one_in(16) || elem.size > 5) {
                mongroup
                    m(GROUP_ZOMBIE,
                      ...
                      static_cast<int>(elem.size * 2.5), elem.size * 80);
        ...
        for (int x = 0; x < OMAPX; x++) {
            for (int y = 0; y < OMAPY; y++) {
                if (ter(tripoint_om_omt(x, y, 0)) == "central_lab_entrance") {
                    add_mon_group(
                        mongroup(GROUP_DIMENSIONAL_SURFACE,
                                 project_to<coords::sm>(tripoint_abs_omt(x, y, 0)), 5, 30));
                }
            }
        }
    }
    ...
    for (int i = 0; i < numgroups; i++) {
        ...
        add_mon_group(
            mongroup(GROUP_WORM, project_combine(pos(), offset), rng(20, 40), rng(30, 50)));
    }
}
...
auto overmap::global_base_point() const -> point_abs_omt { return project_to<coords::omt>(loc); }

void overmap::place_radios() {
    auto strength = []() { return rng(RADIO_MIN_STRENGTH, RADIO_MAX_STRENGTH); };
    std::string message;
    for (int i = 0; i < OMAPX; i++) {
        for (int j = 0; j < OMAPY; j++) {
            ...
            // Since location have id such as "radio_tower_1_north", we must check the beginning of
            // the id
...
auto overmap::open(const dimension_id& dim_id, overmap_special_batch& enabled_specials) -> void {
    ...
    if (g->get_active_world()->read_overmap(dim_id.str(), loc, ter_reader)) {
        const auto plr_reader = [&](std::istream& fin) {
            overmap::
                unserialize_view(fin, string_format("overmap visibility %d.%d", loc.x(), loc.y()));
        };
...
auto overmap::save(const dimension_id& dim_id) const -> void {
    g->get_active_world()
        ->write_overmap_player_visibility(dim_id.str(), loc, [&](std::ostream& stream) {
...
void overmap::add_mon_group(const mongroup& group) {
    ...
    if (group.radius == 1) {
        zg.insert(std::pair<tripoint_om_sm, mongroup>(
            project_remain<coords::om>(group.abs_pos).remainder_tripoint, group));
        return;
    }
    ...
    for (int x = -rad; x <= rad; x++) {
        for (int y = -rad; y <= rad; y++) {
            const int dist =
                group.diffuse
                    ? square_dist(point(x, y), point_zero)
                    : trig_dist(point(x, y), point_zero);
...
void overmap::for_each_npc(const std::function<void(npc&)>& callback) {
    for (auto& guy : npcs) { callback(*guy); }
}
...
auto overmap::find_npc(const character_id& id) const -> shared_ptr_fast<npc> {
    for (const auto& guy : npcs) {
        if (guy->getID() == id) { return guy; }
    }
    return nullptr;
}

auto overmap::is_omt_generated(const tripoint_om_omt& loc) const -> bool {
    ...
    tripoint_abs_sm global_sm_loc = project_to<coords::sm>(project_combine(pos(), loc));
...
void overmap::set_electric_grid_connections(
    const tripoint_om_omt& p, const std::bitset<six_cardinal_directions.size()>& connections) {
    ...
    for (size_t i = 0; i < six_cardinal_directions.size(); i++) {
        ...
        overmap_with_local_coords other =
            get_overmapbuffer(dimension_id_).get_om_global(other_p_global);
        size_t opposite_direction = i + ((i % 2) ? -1 : 1);
        other.om->electric_grid_connections[other.local][opposite_direction] = connections[i];
    }
}

auto overmap_specials::create_building_from(const oter_type_str_id& base) -> overmap_special_id {
    // TODO: Get rid of the hard-coded ids.
...
namespace io {
template <> auto enum_to_string<ot_match_type>(ot_match_type data) -> std::string {
    switch (data) {
        // *INDENT-OFF*
        case ot_match_type::exact:
            return "EXACT";
...
auto oter_get_rotation_dir(const oter_id& oter) -> om_direction::type {
    for (const om_direction::type& rot : om_direction::all) {
        ...
        if (oter.id().str().ends_with(rot_s)) { return rot; }
    }
    return om_direction::type::invalid;
}
