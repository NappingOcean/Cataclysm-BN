...
auto overmapbuffer::get(const point_abs_om& p) -> overmap& {
    {
        ...
        if (it != overmaps.end()) { return *it->second.get(); }
    }
    ...
    for (const point_abs_om& loc : locs) {
        {
            ...
            if (overmaps.count(loc)) { continue; }
        }
...
void overmapbuffer::fix_npcs(overmap& new_overmap) {
    ...
    for (auto& ptr : to_relocate) {
        ...
        if (!has(npc_om_pos)) {
            ...
            debugmsg("NPC %s is out of bounds, on non-generated overmap %s", np.name,
                     loc.to_string());
            point_abs_sm npc_sm = project_to<coords::sm>(npc_om_pos);
            point_abs_sm min = project_to<coords::sm>(loc);
            point_abs_sm max = project_to<coords::sm>(loc + point_south_east) - point_south_east;
...
void overmapbuffer::delete_extra(const tripoint_abs_omt& p) {
    ...
    if (!om_loc) { return; }
    auto lk = std::lock_guard(extras_mutex_);
    if (om_loc.om->has_extra(om_loc.local)) { om_loc.om->delete_extra(om_loc.local); }
}
...
auto overmapbuffer::get_om_global(const tripoint_abs_omt& p) -> overmap_with_local_coords {
    point_abs_om om_pos;
    point_om_omt local;
...
auto overmapbuffer::get_existing_om_global(const point_abs_omt& p) -> overmap_with_local_coords {
    return get_existing_om_global(tripoint_abs_omt(p, 0));
}

auto overmapbuffer::get_existing_om_global(const tripoint_abs_omt& p) -> overmap_with_local_coords {
    point_abs_om om_pos;
    point_om_omt local;
    std::tie(om_pos, local) = project_remain<coords::om>(p.xy());
    overmap* om = get_existing(om_pos);
    if (om == nullptr) { return overmap_with_local_coords{nullptr, tripoint_om_omt()}; }
...
auto overmapbuffer::has_note_with_danger_radius(const tripoint_abs_omt& p) -> std::optional<int> {
    if (const overmap_with_local_coords om_loc = get_existing_om_global(p)) {
...
auto overmapbuffer::is_path(const tripoint_abs_omt& p) -> bool {
    if (const overmap_with_local_coords om_loc = get_existing_om_global(p)) {
...
auto overmapbuffer::has_vehicle(const tripoint_abs_omt& p) -> bool {
    ...
    const overmap_with_local_coords om_loc = get_existing_om_global(p);
    if (!om_loc) { return false; }

    for (const auto& v : om_loc.om->vehicles) {
        if (v.second.p == om_loc.local.xy()) { return true; }
    }
...
auto overmapbuffer::get_vehicle(const tripoint_abs_omt& p) -> std::vector<om_vehicle> {
    ...
    if (p.z() != 0) { return result; }
    const overmap_with_local_coords om_loc = get_existing_om_global(p);
    if (!om_loc) { return result; }
    for (const auto& ov : om_loc.om->vehicles) {
        if (ov.second.p == om_loc.local.xy()) { result.push_back(ov.second); }
    }
    return result;
}
...
void overmapbuffer::signal_hordes(const tripoint_abs_sm& center, const int sig_power) {
    ...
    for (auto& om : get_overmaps_near(center, radius)) { om->signal_hordes(center, sig_power); }
}
...
void overmapbuffer::process_mongroups() {
    ...
    for (auto& om : get_overmaps_near(center, radius)) { om->process_mongroups(); }
}
...
void overmapbuffer::move_hordes() {
    ...
    std::ranges::for_each(overmaps, [](overmap* om) { om->move_hordes(); });
    std::ranges::for_each(overmaps, [this](overmap* om) { fix_mongroups(*om); });
}
...
void overmapbuffer::move_nemesis() {
    ...
    std::ranges::transform(
        overmaps | std::views::values, std::back_inserter(overmap_ptrs),
        [](const std::unique_ptr<overmap>& om) -> overmap* { return om.get(); });
    std::ranges::for_each(overmap_ptrs, [](overmap* om) { om->move_nemesis(); });
    std::ranges::for_each(overmap_ptrs, [this](overmap* om) { fix_nemesis(*om); });
}

void overmapbuffer::remove_nemesis() {
    for (std::pair<const point_abs_om, std::unique_ptr<overmap>>& omp : overmaps) {
        const bool nemesis_removed = omp.second->remove_nemesis();
        if (nemesis_removed) { break; }
    }
}

auto overmapbuffer::create_horde(const mongroup& group) -> mongroup* {
    ...
    auto match = std::ranges::find_if(
        std::ranges::subrange(groups_range.first, groups_range.second), [&](const auto& entry) {
...
auto overmapbuffer::monsters_at(const tripoint_abs_omt& p) -> std::vector<mongroup*> {
    ...
    for (point offset :
         std::array<point, 4>{{{point_zero}, {point_south}, {point_east}, {point_south_east}}}) {
...
auto overmapbuffer::scents_near(const tripoint_abs_omt& origin)
    -> std::array<std::array<scent_trace, 3>, 3> {
...
void overmapbuffer::add_vehicle(vehicle* veh) {
    ...
    while (om_loc.om->vehicles.contains(id)) { id++; }
...
void overmapbuffer::clear_pocket_info() { pocket_info_.reset(); }
...
auto overmapbuffer::join_used_at(const std::pair<tripoint_abs_omt, cube_direction>& p)
    -> std::string* {
    const overmap_with_local_coords om_loc = get_om_global(p.first);
    return om_loc.om->join_used_at({om_loc.local, p.second});
}

auto overmapbuffer::mapgen_args(const tripoint_abs_omt& p) -> std::optional<mapgen_arguments>* {
    const overmap_with_local_coords om_loc = get_om_global(p);
    return om_loc.om->mapgen_args(om_loc.local);
}

auto overmapbuffer::get_or_init_mapgen_args(
...
#else
    const bool already_init = __atomic_load_n(slot.init_flag, __ATOMIC_ACQUIRE) != 0;
#endif
    if (already_init) { return *slot.args; }
...
#else
    const bool still_uninit = __atomic_load_n(slot.init_flag, __ATOMIC_RELAXED) == 0;
#endif
    if (still_uninit) {
...
auto overmapbuffer::reveal(const tripoint_abs_omt& center, int radius) -> bool {
    return reveal(center, radius, [](const oter_id&) { return true; });
}

auto overmapbuffer::reveal(
    const tripoint_abs_omt& center, int radius, const std::function<bool(const oter_id&)>& filter)
    -> bool {
    ...
    for (int i = -radius; i <= radius; i++) {
        for (int j = -radius; j <= radius; j++) {
            ...
            if (seen(p)) { continue; }
            if (trigdist && i * i + j * j > radius_squared) { continue; }
            if (!filter(ter(p))) { continue; }
            result = true;
            set_seen(p, true);
        }
    }
...
auto overmap_path_params::for_land_vehicle(float offroad_coeff, bool tiny, bool amphibious)
    -> overmap_path_params {
...
static auto get_terrain_cost(
    const tripoint_abs_omt& omt_pos, const overmap_path_params& params, overmapbuffer& omb) -> int {
    if (params.only_known_by_player && !omb.seen(omt_pos)) { return -1; }
    if (params.avoid_danger && omb.is_marked_dangerous(omt_pos)) { return -1; }
    ...
    if (is_ot_match("road", oter, ot_match_type::type)
        ...
        || is_ot_match("road_nesw_manhole", oter, ot_match_type::type) || omb.is_path(omt_pos)) {
        return params.road_cost;
    } else if (is_ot_match("field", oter, ot_match_type::type)) {
        return params.field_cost;
    } else if (
        is_ot_match("rural_road", oter, ot_match_type::prefix)
        ...
        is_ot_match("empty_rock", oter, ot_match_type::type)
...
static auto is_ramp(const tripoint_abs_omt& omt_pos, overmapbuffer& omb) -> bool {
    const oter_id& oter = omb.ter_existing(omt_pos);
...
auto overmapbuffer::get_travel_path(
    ...
    if (src == overmap::invalid_tripoint || dest == overmap::invalid_tripoint) { return {}; }

    const pf::omt_scoring_fn estimate = [&](tripoint_abs_omt pos) {
        const int cur_cost = pos == src ? 0 : get_terrain_cost(pos, params, *this);
        if (cur_cost < 0) { return pf::omt_score::rejected; }
        return pf::omt_score(cur_cost, is_ramp(pos, *this));
    };

    constexpr int radius = 4 * OMAPX; // radius of search in OMTs = 4 overmaps
    const pf::simple_path<tripoint_abs_omt> path =
        pf::find_overmap_path(src, dest, radius, estimate);
    return path.points;
}

auto overmapbuffer::reveal_route(
    const tripoint_abs_omt& source, const tripoint_abs_omt& dest, const omt_route_params& params)
    -> bool {
    ...
    if (source == overmap::invalid_tripoint || dest == overmap::invalid_tripoint) { return false; }
    ...
    const auto get_ter_at = [&](const point_rel_omt& p) { return ter(base + p); };
    ...
    const pf::two_node_scoring_fn<point_rel_omt> estimate =
        [&](pf::directed_node<point_rel_omt> cur, std::optional<pf::directed_node<point_rel_omt>>) {
            ...
            if (!connection->has(oter)) {
                if (params.road_only) { return pf::node_score::rejected; }
                if (is_river(oter)) {
    ...
    for (const auto& node : path.nodes) { reveal(base + node.pos, params.radius); }
    return !path.nodes.empty();
}

auto overmapbuffer::check_ot_existing(
    const std::string& type, ot_match_type match_type, const tripoint_abs_omt& loc) -> bool {
...
auto overmapbuffer::overmap_special_at(const tripoint_abs_omt& loc)
    -> std::optional<overmap_special_id> {
    const overmap_with_local_coords om_loc = get_om_global(loc);
    return om_loc.om->overmap_special_at(om_loc.local);
}

auto overmapbuffer::check_ot(
    const std::string& type, ot_match_type match_type, const tripoint_abs_omt& p) -> bool {
    const overmap_with_local_coords om_loc = get_om_global(p);
    return om_loc.om->check_ot(type, match_type, om_loc.local);
}

auto overmapbuffer::check_overmap_special_type(
    const overmap_special_id& id, const tripoint_abs_omt& loc) -> bool {
    const overmap_with_local_coords om_loc = get_om_global(loc);
    return om_loc.om->check_overmap_special_type(id, om_loc.local);
}
...
auto overmapbuffer::contains_unique_special(const overmap_special_id& id) const -> bool {
    return placed_unique_specials.contains(id);
}

auto overmapbuffer::is_findable_location(
    const tripoint_abs_omt& location, const omt_find_params& params) -> bool {
...
auto overmapbuffer::is_findable_location(
    const overmap_with_local_coords& om_loc, const omt_find_params& params) -> bool {
    if (om_loc.om == nullptr) { return false; }

    const auto is_seen = om_loc.om->seen(om_loc.local);
    if (params.seen.has_value() && params.seen.value() != is_seen) { return false; }

    const auto is_explored = om_loc.om->explored(om_loc.local);
    if (params.explored.has_value() && params.explored.value() != is_explored) { return false; }
    ...
    if (params.om_special) {
        bool meets_om_special =
            om_loc.om->check_overmap_special_type(*params.om_special, om_loc.local);
        if (!meets_om_special) { return false; }
    }
...
namespace {
using find_task =
    std::pair<point_abs_om, std::vector<std::pair<tripoint_abs_omt, tripoint_om_omt>>>;
...
struct find_task_generator {
    ...
    auto operator()() -> std::optional<find_task> {
        ...
        for (int n = 0; n < _n_steps; n++) {
            ...
            _current = get_om_loc();
            _done = _end == _it;
            if (_done || _current.first != om_loc) { break; }
        }
...
auto overmapbuffer::find_all(const tripoint_abs_omt& origin, const omt_find_params& params)
    -> std::vector<tripoint_abs_omt> {
...
auto overmapbuffer::find_all_sync(const tripoint_abs_omt& origin, const omt_find_params& params)
    -> std::vector<tripoint_abs_omt> {
    ...
    const auto search_layers = params.search_layers.value_or(
        std::make_pair(origin.z(), origin.z()));
    ...
    while (true) {
        ...
        if (params.popup) { params.popup->refresh(); }

        auto task_data = gen();
        if (!task_data.has_value()) { break; }

        auto& [task_om, task_omts] = task_data.value();
        if (params.existing_only && !has(task_om)) { continue; }
        ...
        for (const auto& loc : task_omts) {
            ...
            if (is_findable_location(q, params)) { find_result.push_back(loc.first); }
...
auto overmapbuffer::find_all_async(const tripoint_abs_omt& origin, const omt_find_params& params)
    -> std::vector<tripoint_abs_omt> {
    // max_dist == 0 means search a whole overmap diameter.
    const int min_dist = params.search_range.first;
    const int max_dist = params.search_range.second ? params.search_range.second : OMAPX;
    ...
    auto try_finish_task =
        [](std::future<std::vector<tripoint_abs_omt>>& task, std::vector<tripoint_abs_omt>& dst,
           omt_find_params params) -> bool {
    ...
    while (true) {
        ...
        auto task_func =
            [&](point_abs_om l, std::vector<std::pair<tripoint_abs_omt, tripoint_om_omt>> locals) {
                ...
                for (const auto& loc : locals) {
                    overmap_with_local_coords q{om_loc, loc.second};
                    if (is_findable_location(q, params)) { result.push_back(loc.first); }
                    if (params.max_results.has_value()
...
auto overmapbuffer::find_closest(const tripoint_abs_omt& origin, const omt_find_params& pp)
    -> tripoint_abs_omt {
    // Check the origin before searching adjacent tiles!
    if (pp.search_range.first == 0 && is_findable_location(origin, pp)) { return origin; }
    ...
    if (params.search_range.second == 0) { params.search_range.second = OMAPX * 5; }
    if (!params.max_results.has_value()) {
    ...
    if (scan_result.empty()) { return overmap::invalid_tripoint; }
...
auto overmapbuffer::find_random(const tripoint_abs_omt& origin, const omt_find_params& params)
    -> tripoint_abs_omt {
...
auto overmapbuffer::find_npc(character_id id) -> shared_ptr_fast<npc> {
    for (auto& it : overmaps) {
        if (auto p = it.second->find_npc(id)) { return p; }
    }
    return nullptr;
}
...
auto overmapbuffer::remove_npc(const character_id& id) -> shared_ptr_fast<npc> {
    ...
    std::lock_guard<std::mutex> lk(npc_mutex_);
    for (auto& it : overmaps) {
        if (const auto p = it.second->erase_npc(id)) { return p; }
    }
...
auto overmapbuffer::get_overmaps_near(const tripoint_abs_sm& location, const int radius)
    -> std::vector<overmap*> {
    ...
    const point_abs_om start = project_to<coords::om>(location.xy() + point(-radius, -radius));
    const point_abs_om end = project_to<coords::om>(location.xy() + point(radius, radius));
    ...
    std::ranges::sort(result, [&center](const overmap* lhs, const overmap* rhs) {
...
auto overmapbuffer::get_overmaps_near(const point_abs_sm& p, const int radius)
    -> std::vector<overmap*> {
...
auto overmapbuffer::get_companion_mission_npcs(int range) -> std::vector<shared_ptr_fast<npc>> {
    ...
    for (const auto& guy : get_npcs_near_player(range)) {
        if (guy->has_companion_mission()) { available.push_back(guy); }
    }
    return available;
}

// If z == INT_MIN, allow all z-levels
auto overmapbuffer::get_npcs_near(const tripoint_abs_sm& p, int radius)
    ...
    for (auto& it : nearby) {
        auto temp = it->get_npcs([&](const npc& guy) {
            ...
            if (p.z() != INT_MIN && pos.z() != p.z()) { return false; }
...
// If z == INT_MIN, allow all z-levels
auto overmapbuffer::get_npcs_near_omt(const tripoint_abs_omt& p, int radius)
    -> std::vector<shared_ptr_fast<npc>> {
...
static auto create_radio_tower_reference(
    const overmap& om, radio_tower& t, const tripoint_abs_sm& center) -> radio_tower_reference {
...
auto overmapbuffer::find_radio_station(const int frequency) -> radio_tower_reference {
    ...
    for (auto& om : get_overmaps_near(center, RADIO_MAX_STRENGTH)) {
        for (auto& tower : om->radios) {
            ...
            if (rref.signal_strength > 0 && tower.frequency == frequency) { return rref; }
        }
    }
...
auto overmapbuffer::find_all_radio_stations() -> std::vector<radio_tower_reference> {
    ...
    for (auto& om : get_overmaps_near(center, radius)) {
        for (auto& tower : om->radios) {
            const auto rref = create_radio_tower_reference(*om, tower, center);
            if (rref.signal_strength > 0) { result.push_back(rref); }
        }
    }
    return result;
}
...
auto overmapbuffer::get_overmap_npcs() -> std::vector<shared_ptr_fast<npc>> {
    ...
    for (auto& om : overmaps) {
        ...
        for (auto& guy : overmap.npcs) { result.push_back(guy); }
    }
    return result;
}

auto overmapbuffer::get_cities_near(const tripoint_abs_sm& location, int radius)
    ...
    for (const auto om : get_overmaps_near(location, radius)) {
        ...
        std::ranges::transform(om->cities, std::back_inserter(result), [&](city& element) {
            const auto rel_pos_city = project_to<coords::sm>(element.pos);
            const auto abs_pos_city = tripoint_abs_sm(project_combine(om->pos(), rel_pos_city), 0);
    ...
    std::ranges::sort(result, [](const city_reference& lhs, const city_reference& rhs) {
...
auto overmapbuffer::closest_known_city(const tripoint_abs_sm& center) -> city_reference {
    constexpr int radius =
        ...
        / coords::map_squares_per(coords::scale::submap);
    const auto cities = get_cities_near(center, radius);
    const auto it = std::ranges::find_if(cities, [this](const city_reference& elem) {
        const tripoint_abs_omt p = project_to<coords::omt>(elem.abs_sm_pos);
        return seen(p);
    });

    if (it != cities.end()) { return *it; }
...
auto overmapbuffer::get_description_at(const tripoint_abs_sm& where) -> std::string {
    ...
    if (where.z() != 0) { return ter_name; }

    const auto closest_cref = closest_known_city(where);

    if (!closest_cref) { return ter_name; }
    ...
    //~ First parameter is a terrain name, second parameter is a direction, and third parameter is a
    // city name.
    ...
    if (sm_dist <= 3 * sm_size / 4) {
        if (sm_size >= 16) {
            ...
            if (sm_dist <= sm_size / 4) {
                //~ First parameter is a terrain name, second parameter is a direction, and third
                // parameter is a city name.
...
void overmapbuffer::spawn_monster(const tripoint_abs_sm& p) {
    ...
    std::for_each(
        monster_bucket.first, monster_bucket.second,
        [&](std::pair<const tripoint_om_sm, monster>& monster_entry) {
            ...
            if (!here.inbounds(local)) {
                debugmsg(
                    "Monster at bub( %s, %s, %s ), abs( %s, %s, %s ) was out of bounds. Skipping spawn",
                    local.x(), local.y(), local.z(), ms.x(), ms.y(), ms.z());
            }
            monster* const placed =
                g->place_critter_at(make_shared_fast<monster>(this_monster), local);
...
auto overmapbuffer::get_notes(int z, const std::string* pattern) -> overmapbuffer::t_notes_vector {
    ...
    for (auto& it : overmaps) {
        ...
        for (const om_note& note : all_om_notes) {
            if (pattern != nullptr && lcmatch(note.text, *pattern)) { continue; }
            result.emplace_back(project_combine(it.first, note.p), note.text);
        }
    }
    return result;
}

auto overmapbuffer::get_extras(int z, const std::string* pattern)
    -> overmapbuffer::t_extras_vector {
...
auto overmapbuffer::is_safe(const tripoint_abs_omt& p) -> bool {
    for (auto& mongrp : monsters_at(p)) {
        if (!mongrp->is_safe()) { return false; }
    }
    return true;
}
...
auto overmapbuffer::place_special(
    ...
    -> std::optional<std::vector<tripoint_abs_omt>> {
    ...
    if (om_loc.om->can_place_special(special, om_loc.local, dir, must_be_unexplored) || force) {
        ...
        std::vector<tripoint_abs_omt> result;
        for (const tripoint_om_omt& p :
             om_loc.om->place_special(special, om_loc.local, dir, c, must_be_unexplored, force)) {
...
auto overmapbuffer::place_special(
    ...
    if (!special_id.is_valid()) { return false; }
    ...
    for (const auto& om :
         get_overmaps_near(project_to<coords::sm>(center),
        ...
        for (const tripoint_abs_omt& p : closest_points_first(center, min, max)) {
            ...
            if (overmap == om->pos()) { points_in_range.push_back(omt_within_overmap); }
        }

        // Attempt to place the specials using filtered points. We
        // require they be placed in unexplored terrain right now.
        if (om->place_special_custom(special, points_in_range) > 0) { return true; }
    }
...
auto overmapbuffer::electric_grid_at(const tripoint_abs_omt& p) -> std::set<tripoint_abs_omt> {
    ...
    while (!open.empty()) {
        ...
        const auto& connections_bitset =
            cached_om->electric_grid_connections[tripoint_om_omt(local_2d, elem.z())];
        for (size_t i = 0; i < six_cardinal_directions.size(); i++) {
            if (connections_bitset.test(i)) {
                ...
                if (visited.emplace(other).second) { open.emplace(other); }
            }
        }
    }
...
auto overmapbuffer::electric_grid_connectivity_at(const tripoint_abs_omt& p)
    ...
    for (size_t i = 0; i < six_cardinal_directions.size(); i++) {
        if (connections_bitset.test(i)) { ret.emplace_back(six_cardinal_directions[i]); }
    }

    return ret;
}

auto overmapbuffer::add_grid_connection(const tripoint_abs_omt& lhs, const tripoint_abs_omt& rhs)
    -> bool {
    ...
    const auto lhs_iter = std::ranges::
        find(six_cardinal_directions,

             coord_diff.raw());
    const auto rhs_iter = std::ranges::
        find(six_cardinal_directions,
...
auto overmapbuffer::remove_grid_connection(const tripoint_abs_omt& lhs, const tripoint_abs_omt& rhs)
    -> bool {
    const tripoint_rel_omt coord_diff = rhs - lhs;
    if (std::abs(coord_diff.x()) + std::abs(coord_diff.y()) + std::abs(coord_diff.z()) != 1) {
